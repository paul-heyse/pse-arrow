...
def _model_record(item: Item) -> dict:
    ...
    # Emitted only when it is not the ordinary case, so a model record stays readable and a
    # rule that asks for the exceptions does not have to filter the majority out.
    if item.visibility != "supported":
        record["visibility"] = item.visibility
    if item.reached_via:
        record["reached_via"] = item.reached_via
    if item.deprecated:
    ...
    if item.methods:
        ...
        for method in sorted(item.methods, key=lambda m: (m.name, m.via_trait or "")):
            ...
            if method.visibility != "supported":
                entry["visibility"] = method.visibility
            if method.via_trait:
...
def write_provenance(
    ...
    corpora: dict[str, dict] | None = None,
    ...
    # Corpus provenance belongs in the published artifact, not only in the manifest that built
    # it. Without this block `content/` cannot say which commit of upstream it describes, which
    # is a gap the sibling repositories have and a reader cannot close from the outside.
    if corpora is not None:
        provenance["corpora"] = corpora

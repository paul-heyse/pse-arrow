...
async fn partition_preview_cap_and_live_stream_lifetime() -> Result<()> {
    ...
    assert_eq!(count, 6);
    // In this resolved DataFusion profile the outer adapter drops its inner
    // recording stream on EOF, while the public adapter handles remain alive.
    assert!(!c.rows("close", "InstrumentedExec").is_empty());
    c.save("partition-preview-at-eof");
    let at_eof = c.rows("close", "InstrumentedExec").len();
    drop(streams);
    assert_eq!(c.rows("close", "InstrumentedExec").len(), at_eof);

...
def write(root: Path, content: Path) -> dict[str, int]:
    ...
    for name, expected in receipt.get("captures", {}).items():
        if not (root / name).is_file() or digest(root / name) != expected:
            raise ValueError(f"Probe capture changed since execution: {name}")
    for path in sorted((root / "authoring/capabilities").glob("*.json")):
        ...
        for common in ["build/manifests/datafusion-tracing.json", "content/index/features.tsv"]:
        ...
        for evidence in record["evidence"]:
            if evidence["kind"] == "runtime_observation":
                ...
                dependency_files.update(receipt.get("captures", {}))
                profile = "skill_improvement/evidence/implementation/probe-profile.json"
                dependency_files[profile] = digest(root / profile)
    ...
    for name, title, facets in [
        (
            "lifecycle",
            "Lifecycle routes",
            ["planning", "execution", "completion", "async", "storage", "export"],
        ),
        (
            "symptoms",
            "Symptoms and discriminating evidence",
            ["filter", "field-schema", "preview", "metrics", "lifecycle", "versions"],
        ),
    ]:
        lines = [f"# {title}", "", "Each route exposes alternatives and evidence limits.", ""]
        for facet in facets:
            lines.extend([f"## {facet}", ""])
            lines.extend(
                f"- [{r['title']}](../capabilities/{r['id']}.md): {r['brief']}"
                for r in records
                if facet in r["properties"]
            )
            lines.append("")
        (routes / f"{name}.md").write_text("\n".join(lines))
    coverage = {

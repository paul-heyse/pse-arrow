...
decision rules. This profile adds principles, gates, slot content and lenses for process
simulation software.
...
2. **The physical-semantics table and well-posedness statement** (slots 2–3), reconstructed
   from the subject. Cells you had to invent are unresolved decisions.
3. **Numerical stage columns in slot 4** for every stage that formulates, evaluates or solves.
4. **Simulator journeys in slot 5**, chosen by relevance. The workloads in the profile's
   functional target — edit and re-solve, studies, recycles, dynamics, extension — are in scope
   for a target-purpose review even when current plans exclude them.

...
Full upstream contracts; raw type trees and source locators in [structured records](datafusion_expr_common.groups_accumulator.GroupsAccumulator.json).

<a id="op-9c9c43a7d8bf357eb3adcb6e"></a>
## GroupsAccumulator

`trait` · `datafusion_expr_common::groups_accumulator::GroupsAccumulator` · datafusion-expr-common 55.1.0

```rust
trait GroupsAccumulator: Send + std::any::Any
```

Source: `src/groups_accumulator.rs:105`. [Exact documentation build](https://docs.rs/crate/datafusion-expr-common/55.1.0/json).

`GroupsAccumulator` implements a single aggregate (e.g. AVG) and
stores the state for *all* groups internally.

Logically, a [`GroupsAccumulator`](../operations/datafusion_expr_common.groups_accumulator.GroupsAccumulator.md#op-9c9c43a7d8bf357eb3adcb6e) stores a mapping from each group index to
...
<a id="op-0d4ea64ca8d09e96d56b2339"></a>
## convert_to_state

`function` · `datafusion_expr_common::groups_accumulator::GroupsAccumulator::convert_to_state` · datafusion-expr-common 55.1.0

```rust
fn convert_to_state(&self, values: &[ArrayRef], opt_filter: Option<&BooleanArray>) -> Result<Vec<ArrayRef>>
```

Source: `src/groups_accumulator.rs:232`. [Exact documentation build](https://docs.rs/crate/datafusion-expr-common/55.1.0/json).
...
<a id="op-0028ea05f0ec12af4fec2e33"></a>
## evaluate

`function` · `datafusion_expr_common::groups_accumulator::GroupsAccumulator::evaluate` · datafusion-expr-common 55.1.0

```rust
fn evaluate(&mut self, emit_to: EmitTo) -> Result<ArrayRef>
```

Source: `src/groups_accumulator.rs:155`. [Exact documentation build](https://docs.rs/crate/datafusion-expr-common/55.1.0/json).
...
If `emit_to` is [`EmitTo::All`](../operations/datafusion_expr_common.groups_accumulator.EmitTo.md#op-3675659b93702fc0d63f9c0b), the accumulator should
return all groups and release / reset its internal state
equivalent to when it was first created.

If `emit_to` is [`EmitTo::First`](../operations/datafusion_expr_common.groups_accumulator.EmitTo.md#op-935d538c0b51c7f0267f586c), only the first `n` groups
...
`n`. See [`EmitTo::First`](../operations/datafusion_expr_common.groups_accumulator.EmitTo.md#op-935d538c0b51c7f0267f586c) for more details.

<a id="op-ac4aa90855289638c8197fda"></a>
## merge_batch

`function` · `datafusion_expr_common::groups_accumulator::GroupsAccumulator::merge_batch` · datafusion-expr-common 55.1.0

```rust
fn merge_batch(&mut self, values: &[ArrayRef], group_indices: &[usize], total_num_groups: usize) -> Result<()>
```

Source: `src/groups_accumulator.rs:190`. [Exact documentation build](https://docs.rs/crate/datafusion-expr-common/55.1.0/json).

Merges intermediate state (the output from [`Self::state`](../operations/datafusion_expr_common.groups_accumulator.GroupsAccumulator.md#op-8a9d88d91cc4945f7fa28591))
...
differ. See [`Self::state`](../operations/datafusion_expr_common.groups_accumulator.GroupsAccumulator.md#op-8a9d88d91cc4945f7fa28591) for more details on how state is
used and merged.

* `values`: arrays produced from previously calling `state` on other accumulators.

Other arguments are the same as for [`Self::update_batch`](../operations/datafusion_expr_common.groups_accumulator.GroupsAccumulator.md#op-87bd9abfc36bda6d73091c12), except that
...
<a id="op-871d63ca775fd70fca8e6d29"></a>
## size

`function` · `datafusion_expr_common::groups_accumulator::GroupsAccumulator::size` · datafusion-expr-common 55.1.0

```rust
fn size(&self) -> usize
```

Source: `src/groups_accumulator.rs:245`. [Exact documentation build](https://docs.rs/crate/datafusion-expr-common/55.1.0/json).
...
<a id="op-8a9d88d91cc4945f7fa28591"></a>
## state

`function` · `datafusion_expr_common::groups_accumulator::GroupsAccumulator::state` · datafusion-expr-common 55.1.0

```rust
fn state(&mut self, emit_to: EmitTo) -> Result<Vec<ArrayRef>>
```

Source: `src/groups_accumulator.rs:173`. [Exact documentation build](https://docs.rs/crate/datafusion-expr-common/55.1.0/json).
...
See [`Self::evaluate`](../operations/datafusion_expr_common.groups_accumulator.GroupsAccumulator.md#op-0028ea05f0ec12af4fec2e33) for details on the required output
order and `emit_to`.

[`Accumulator::state`]: crate::accumulator::Accumulator::state

<a id="op-87bd9abfc36bda6d73091c12"></a>
## update_batch

`function` · `datafusion_expr_common::groups_accumulator::GroupsAccumulator::update_batch` · datafusion-expr-common 55.1.0
...
Source: `src/groups_accumulator.rs:126`. [Exact documentation build](https://docs.rs/crate/datafusion-expr-common/55.1.0/json).

Updates the accumulator's state from its arguments, encoded as
a vector of [`ArrayRef`](../operations/arrow_array.array.ArrayRef.md#op-657cc3ff3d24afcacda590e1)s.

...
Crate `sqlparser` · 237 public items · structured records in [`model/sqlparser.ast.json`](../model/sqlparser.ast.json)

## AccessExpr

`enum` · `sqlparser::ast::AccessExpr`

```rust
enum AccessExpr
```

**Variants**: `Dot`, `Subscript`
...
`enum` · `sqlparser::ast::Action`

```rust
enum Action
```

**Variants**: `AddSearchOptimization`, `Apply`, `ApplyBudget`, `AttachListing`, `AttachPolicy`, `Audit`, `BindServiceEndpoint`, `Connect`, `Create`, `DatabaseRole`, `Delete`, `Drop`, `EvolveSchema`, `Exec`, `Execute`, `Failover`, `ImportedPrivileges`, `ImportShare`, `Insert`, `Manage`, `ManageReleases`, `ManageVersions`, `Modify`, `Monitor`, `Operate`, `OverrideShareRestrictions`, `Ownership`, `PurchaseDataExchangeListing`, `Read`, `ReadSession`, `References`, `Replicate`, `ResolveAll`, `Role`, `Select`, `Temporary`, `Trigger`, `Truncate`, `Update`, `Usage`
...
`enum` · `sqlparser::ast::ActionApplyType`

```rust
enum ActionApplyType
```

**Variants**: `AggregationPolicy`, `AuthenticationPolicy`, `JoinPolicy`, `MaskingPolicy`, `PackagesPolicy`, `PasswordPolicy`, `ProjectionPolicy`, `RowAccessPolicy`, `SessionPolicy`, `Tag`
...
`enum` · `sqlparser::ast::ActionCreateObjectType`

```rust
enum ActionCreateObjectType
```

**Variants**: `Account`, `Application`, `ApplicationPackage`, `ComputePool`, `DataExchangeListing`, `Database`, `ExternalVolume`, `FailoverGroup`, `Integration`, `NetworkPolicy`, `OrganiationListing`, `ReplicationGroup`, `Role`, `Schema`, `Share`, `User`, `Warehouse`
...
`enum` · `sqlparser::ast::ActionExecuteObjectType`

```rust
enum ActionExecuteObjectType
```

**Variants**: `Alert`, `DataMetricFunction`, `ManagedAlert`, `ManagedTask`, `Task`
...
`enum` · `sqlparser::ast::ActionManageType`

```rust
enum ActionManageType
```

**Variants**: `AccountSupportCases`, `EventSharing`, `Grants`, `ListingAutoFulfillment`, `OrganizationSupportCases`, `UserSupportCases`, `Warehouses`
...
`enum` · `sqlparser::ast::ActionModifyType`

```rust
enum ActionModifyType
```

**Variants**: `LogLevel`, `TraceLevel`, `SessionLogLevel`, `SessionTraceLevel`
...
`enum` · `sqlparser::ast::ActionMonitorType`

```rust
enum ActionMonitorType
```

**Variants**: `Execution`, `Security`, `Usage`
...
`enum` · `sqlparser::ast::AddDropSync`

```rust
enum AddDropSync
```

**Variants**: `ADD`, `DROP`, `SYNC`
...
**Variants**: `TEXT`, `GRAPHVIZ`, `JSON`, `TRADITIONAL`, `TREE`
...
`enum` · `sqlparser::ast::AnalyzeFormatKind`

```rust
enum AnalyzeFormatKind
```

**Variants**: `Keyword`, `Assignment`
...
`enum` · `sqlparser::ast::ArgMode`

```rust
enum ArgMode
```

**Variants**: `In`, `Out`, `InOut`, `Variadic`
...
`enum` · `sqlparser::ast::AssignmentTarget`

```rust
enum AssignmentTarget
```

**Variants**: `ColumnName`, `Tuple`
...
`enum` · `sqlparser::ast::AttachDuckDBDatabaseOption`

```rust
enum AttachDuckDBDatabaseOption
```

**Variants**: `ReadOnly`, `Type`
...
`enum` · `sqlparser::ast::BeginTransactionKind`

```rust
enum BeginTransactionKind
```

**Variants**: `Transaction`, `Work`, `Tran`
...
`enum` · `sqlparser::ast::CascadeOption`

```rust
enum CascadeOption
```

**Variants**: `Cascade`, `Restrict`
...
`enum` · `sqlparser::ast::CastFormat`

```rust
enum CastFormat
```

**Variants**: `Value`, `ValueAtTimeZone`
...
`enum` · `sqlparser::ast::CastKind`

```rust
enum CastKind
```

**Variants**: `Cast`, `TryCast`, `SafeCast`, `DoubleColon`
...
`enum` · `sqlparser::ast::CatalogSyncNamespaceMode`

```rust
enum CatalogSyncNamespaceMode
```

**Variants**: `Nest`, `Flatten`
...
`enum` · `sqlparser::ast::CeilFloorKind`

```rust
enum CeilFloorKind
```

**Variants**: `DateTimeField`, `Scale`
...
`enum` · `sqlparser::ast::CommentDef`

```rust
enum CommentDef
```

**Variants**: `WithEq`, `WithoutEq`
...
**Variants**: `Collation`, `Column`, `Database`, `Domain`, `Extension`, `Function`, `Index`, `MaterializedView`, `Procedure`, `Role`, `Schema`, `Sequence`, `Table`, `Type`, `User`, `View`
...
`enum` · `sqlparser::ast::ConditionalStatements`

```rust
enum ConditionalStatements
```

**Variants**: `Sequence`, `BeginEnd`
...
`enum` · `sqlparser::ast::ConstraintReferenceMatchKind`

```rust
enum ConstraintReferenceMatchKind
```

**Variants**: `Full`, `Partial`, `Simple`
...
`enum` · `sqlparser::ast::CopyIntoSnowflakeKind`

```rust
enum CopyIntoSnowflakeKind
```

**Variants**: `Table`, `Location`
...
**Variants**: `Header`, `Quote`, `Escape`, `ForceQuote`, `ForceNotNull`
...
**Variants**: `AcceptAnyDate`, `AcceptInvChars`, `AddQuotes`, `AllowOverwrite`, `Binary`, `BlankAsNull`, `Bzip2`, `CleanPath`, `CompUpdate`, `Csv`, `DateFormat`, `Delimiter`, `EmptyAsNull`, `Encrypted`, `Escape`, `Extension`, `FixedWidth`, `Gzip`, `Header`, `IamRole`, `IgnoreHeader`, `Json`, `Manifest`, `MaxFileSize`, `Null`, `Parallel`, `Parquet`, `PartitionBy`, `Region`, `RemoveQuotes`, `RowGroupSize`, `StatUpdate`, `TimeFormat`, `TruncateColumns`, `Zstd`, `Credentials`
...
**Variants**: `Format`, `Freeze`, `Delimiter`, `Null`, `Header`, `Quote`, `Escape`, `ForceQuote`, `ForceNotNull`, `ForceNull`, `Encoding`
...
**Variants**: `Stdin`, `Stdout`, `File`, `Program`
...
`enum` · `sqlparser::ast::CreateFunctionBody`

```rust
enum CreateFunctionBody
```

**Variants**: `AsBeforeOptions`, `AsAfterOptions`, `AsBeginEnd`, `Return`, `AsReturnExpr`, `AsReturnSelect`
...
`enum` · `sqlparser::ast::CreateTableLikeDefaults`

```rust
enum CreateTableLikeDefaults
```

**Variants**: `Including`, `Excluding`
...
`enum` · `sqlparser::ast::CreateTableLikeKind`

```rust
enum CreateTableLikeKind
```

**Variants**: `Parenthesized`, `Plain`
...
`enum` · `sqlparser::ast::CreateTableOptions`

```rust
enum CreateTableOptions
```

**Variants**: `None`, `With`, `Options`, `Plain`, `TableProperties`
...
`enum` · `sqlparser::ast::CreateViewAlgorithm`

```rust
enum CreateViewAlgorithm
```

**Variants**: `Undefined`, `Merge`, `TempTable`
...
`enum` · `sqlparser::ast::CreateViewSecurity`

```rust
enum CreateViewSecurity
```

**Variants**: `Definer`, `Invoker`
...
`enum` · `sqlparser::ast::CurrentGrantsKind`

```rust
enum CurrentGrantsKind
```

**Variants**: `CopyCurrentGrants`, `RevokeCurrentGrants`
...
`enum` · `sqlparser::ast::DeclareAssignment`

```rust
enum DeclareAssignment
```

**Variants**: `Expr`, `Default`, `DuckAssignment`, `For`, `MsSqlAssignment`
...
`enum` · `sqlparser::ast::DeclareType`

```rust
enum DeclareType
```

**Variants**: `Cursor`, `ResultSet`, `Exception`
...
`enum` · `sqlparser::ast::DescribeAlias`

```rust
enum DescribeAlias
```

**Variants**: `Describe`, `Explain`, `Desc`
...
**Variants**: `ALL`, `PLANS`, `SEQUENCES`, `TEMP`
...
`enum` · `sqlparser::ast::DropFunctionOption`
...
`enum` · `sqlparser::ast::DuplicateTreatment`

```rust
enum DuplicateTreatment
```

**Variants**: `Distinct`, `All`
...
**Variants**: `Identifier`, `CompoundIdentifier`, `CompoundFieldAccess`, `JsonAccess`, `IsFalse`, `IsNotFalse`, `IsTrue`, `IsNotTrue`, `IsNull`, `IsNotNull`, `IsUnknown`, `IsNotUnknown`, `IsDistinctFrom`, `IsNotDistinctFrom`, `IsNormalized`, `InList`, `InSubquery`, `InUnnest`, `Between`, `BinaryOp`, `Like`, `ILike`, `SimilarTo`, `RLike`, `AnyOp`, `AllOp`, `UnaryOp`, `Convert`, `Cast`, `AtTimeZone`, `Extract`, `Ceil`, `Floor`, `Position`, `Substring`, `Trim`, `Overlay`, `Collate`, `Nested`, `Value`, `Prefixed`, `TypedString`, `Function`, `Case`, `Exists`, `Subquery`, `GroupingSets`, `Cube`, `Rollup`, `Tuple`, `Struct`, `Named`, `Dictionary`, `Map`, `Array`, `Interval`, `MatchAgainst`, `Wildcard`, `QualifiedWildcard`, `OuterJoin`, `Prior`, `Lambda`, `MemberOf`
...
`enum` · `sqlparser::ast::ExtractSyntax`
...
**Variants**: `Count`, `Next`, `Prior`, `First`, `Last`, `Absolute`, `Relative`, `All`, `Forward`, `ForwardAll`, `Backward`, `BackwardAll`
...
`enum` · `sqlparser::ast::FetchPosition`
...
**Variants**: `TEXTFILE`, `SEQUENCEFILE`, `ORC`, `PARQUET`, `AVRO`, `RCFILE`, `JSONFILE`
...
`enum` · `sqlparser::ast::FileSizeUnit`
...
`enum` · `sqlparser::ast::FlushLocation`

```rust
enum FlushLocation
```

**Variants**: `NoWriteToBinlog`, `Local`
...
`enum` · `sqlparser::ast::FlushType`

```rust
enum FlushType
```

**Variants**: `BinaryLogs`, `EngineLogs`, `ErrorLogs`, `GeneralLogs`, `Hosts`, `Logs`, `Privileges`, `OptimizerCosts`, `RelayLogs`, `SlowLogs`, `Status`, `UserResources`, `Tables`
...
`enum` · `sqlparser::ast::FromTable`

```rust
enum FromTable
```

**Variants**: `WithFromKeyword`, `WithoutKeyword`
...
**Variants**: `Named`, `ExprNamed`, `Unnamed`
...
**Variants**: `Expr`, `QualifiedWildcard`, `Wildcard`, `WildcardWithOptions`
...
`enum` · `sqlparser::ast::FunctionArgOperator`

```rust
enum FunctionArgOperator
```

**Variants**: `Equals`, `RightArrow`, `Assignment`, `Colon`, `Value`
...
`enum` · `sqlparser::ast::FunctionArgumentClause`

```rust
enum FunctionArgumentClause
```

**Variants**: `IgnoreOrRespectNulls`, `OrderBy`, `Limit`, `OnOverflow`, `Having`, `Separator`, `JsonNullClause`, `JsonReturningClause`
...
`enum` · `sqlparser::ast::FunctionArguments`

```rust
enum FunctionArguments
```

**Variants**: `None`, `Subquery`, `List`
...
`enum` · `sqlparser::ast::FunctionCalledOnNull`

```rust
enum FunctionCalledOnNull
```

**Variants**: `CalledOnNullInput`, `ReturnsNullOnNullInput`, `Strict`
...
`enum` · `sqlparser::ast::FunctionDeterminismSpecifier`

```rust
enum FunctionDeterminismSpecifier
```

**Variants**: `Deterministic`, `NotDeterministic`
...
`enum` · `sqlparser::ast::FunctionParallel`

```rust
enum FunctionParallel
```

**Variants**: `Unsafe`, `Restricted`, `Safe`
...
`enum` · `sqlparser::ast::FunctionSecurity`
...
`enum` · `sqlparser::ast::FunctionSetValue`

```rust
enum FunctionSetValue
```

**Variants**: `Default`, `Values`, `FromCurrent`
...
**Variants**: `AllSequencesInSchema`, `AllTablesInSchema`, `AllViewsInSchema`, `AllMaterializedViewsInSchema`, `AllExternalTablesInSchema`, `AllFunctionsInSchema`, `FutureSchemasInDatabase`, `FutureTablesInSchema`, `FutureViewsInSchema`, `FutureExternalTablesInSchema`, `FutureMaterializedViewsInSchema`, `FutureSequencesInSchema`, `Databases`, `Schemas`, `Sequences`, `Tables`, `Views`, `Warehouses`, `Integrations`, `ResourceMonitors`, `Users`, `ComputePools`, `Connections`, `FailoverGroup`, `ReplicationGroup`, `ExternalVolumes`, `Procedure`, `Function`
...
`enum` · `sqlparser::ast::GranteeName`

```rust
enum GranteeName
```

**Variants**: `ObjectName`, `UserHost`
...
`enum` · `sqlparser::ast::GranteesType`

```rust
enum GranteesType
```

**Variants**: `Role`, `Share`, `User`, `Group`, `Public`, `DatabaseRole`, `Application`, `ApplicationRole`, `None`
...
`enum` · `sqlparser::ast::HavingBoundKind`
...
`enum` · `sqlparser::ast::HiveDelimiter`

```rust
enum HiveDelimiter
```

**Variants**: `FieldsTerminatedBy`, `FieldsEscapedBy`, `CollectionItemsTerminatedBy`, `MapKeysTerminatedBy`, `LinesTerminatedBy`, `NullDefinedAs`
...
`enum` · `sqlparser::ast::HiveDescribeFormat`

```rust
enum HiveDescribeFormat
```

**Variants**: `Extended`, `Formatted`
...
`enum` · `sqlparser::ast::HiveDistributionStyle`

```rust
enum HiveDistributionStyle
```

**Variants**: `PARTITIONED`, `SKEWED`, `NONE`
...
`enum` · `sqlparser::ast::IamRoleKind`

```rust
enum IamRoleKind
```

**Variants**: `Default`, `Arn`
...
`enum` · `sqlparser::ast::InitializeKind`

```rust
enum InitializeKind
```

**Variants**: `OnCreate`, `OnSchedule`
...
`enum` · `sqlparser::ast::JsonNullClause`

```rust
enum JsonNullClause
```

**Variants**: `NullOnNull`, `AbsentOnNull`
...
`enum` · `sqlparser::ast::JsonPathElem`

```rust
enum JsonPathElem
```

**Variants**: `Dot`, `Bracket`, `ColonBracket`
...
`enum` · `sqlparser::ast::KillType`

```rust
enum KillType
```

**Variants**: `Connection`, `Query`, `Mutation`
...
`enum` · `sqlparser::ast::LambdaSyntax`

```rust
enum LambdaSyntax
```

**Variants**: `Arrow`, `LambdaKeyword`
...
`enum` · `sqlparser::ast::ListAggOnOverflow`

```rust
enum ListAggOnOverflow
```

**Variants**: `Error`, `Truncate`
...
`enum` · `sqlparser::ast::LockTableMode`

```rust
enum LockTableMode
```

**Variants**: `AccessShare`, `RowShare`, `RowExclusive`, `ShareUpdateExclusive`, `Share`, `ShareRowExclusive`, `Exclusive`, `AccessExclusive`
...
`enum` · `sqlparser::ast::LockTableType`
...
`enum` · `sqlparser::ast::MacroDefinition`
...
`enum` · `sqlparser::ast::MfaMethodKind`

```rust
enum MfaMethodKind
```

**Variants**: `PassKey`, `Totp`, `Duo`
...
`enum` · `sqlparser::ast::MySQLColumnPosition`

```rust
enum MySQLColumnPosition
```

**Variants**: `First`, `After`
...
**Variants**: `LowPriority`, `Delayed`, `HighPriority`
...
`enum` · `sqlparser::ast::NullInclusion`

```rust
enum NullInclusion
```

**Variants**: `IncludeNulls`, `ExcludeNulls`
...
`enum` · `sqlparser::ast::NullTreatment`
...
`enum` · `sqlparser::ast::ObjectNamePart`

```rust
enum ObjectNamePart
```

**Variants**: `Identifier`, `Function`
...
**Variants**: `Collation`, `Table`, `View`, `MaterializedView`, `Index`, `Schema`, `Database`, `Role`, `Sequence`, `Stage`, `Type`, `User`, `Stream`
...
`enum` · `sqlparser::ast::OnCommit`

```rust
enum OnCommit
```

**Variants**: `DeleteRows`, `PreserveRows`, `Drop`
...
`enum` · `sqlparser::ast::OnInsert`

```rust
enum OnInsert
```

**Variants**: `DuplicateKeyUpdate`, `OnConflict`
...
`enum` · `sqlparser::ast::OneOrManyWithParens`
...
**Implements**: `core::convert::AsRef`, `core::fmt::Display`, `core::iter::traits::collect::IntoIterator`, `core::ops::deref::Deref`, `serde_core::de::Deserialize`, `serde_core::ser::Serialize`, `sqlparser::ast::visitor::Visit`, `sqlparser::ast::visitor::VisitMut`
...
`enum` · `sqlparser::ast::OptimizerHintStyle`
...
`enum` · `sqlparser::ast::PartitionRangeDirection`
...
`enum` · `sqlparser::ast::Password`

```rust
enum Password
```

**Variants**: `Password`, `NullPassword`
...
`enum` · `sqlparser::ast::RaisErrorOption`

```rust
enum RaisErrorOption
```

**Variants**: `Log`, `NoWait`, `SetError`
...
`enum` · `sqlparser::ast::RaiseStatementValue`

```rust
enum RaiseStatementValue
```

**Variants**: `UsingMessage`, `Expr`
...
`enum` · `sqlparser::ast::RefreshModeKind`

```rust
enum RefreshModeKind
```

**Variants**: `Auto`, `Full`, `Incremental`
...
`enum` · `sqlparser::ast::Reset`

```rust
enum Reset
```

**Variants**: `ALL`, `ConfigurationParameter`
...
`enum` · `sqlparser::ast::ReturnStatementValue`
...
**Variants**: `Simple`, `UnnamedAuthorization`, `NamedAuthorization`
...
**Variants**: `InNaturalLanguageMode`, `InNaturalLanguageModeWithQueryExpansion`, `InBooleanMode`, `WithQueryExpansion`
...
**Variants**: `IncrementBy`, `MinValue`, `MaxValue`, `StartWith`, `Cache`, `Cycle`
...
`enum` · `sqlparser::ast::SessionParamStatsTopic`

```rust
enum SessionParamStatsTopic
```

**Variants**: `IO`, `Profile`, `Time`, `Xml`
...
`enum` · `sqlparser::ast::SessionParamValue`
...
`enum` · `sqlparser::ast::Set`

```rust
enum Set
```

**Variants**: `SingleAssignment`, `ParenthesizedAssignments`, `MultipleAssignments`, `SetSessionAuthorization`, `SetSessionParam`, `SetRole`, `SetTimeZone`, `SetNames`, `SetNamesDefault`, `SetTransaction`
...
`enum` · `sqlparser::ast::SetSessionAuthorizationParamKind`

```rust
enum SetSessionAuthorizationParamKind
```

**Variants**: `Default`, `User`
...
`enum` · `sqlparser::ast::SetSessionParamKind`

```rust
enum SetSessionParamKind
```

**Variants**: `Generic`, `IdentityInsert`, `Offsets`, `Statistics`
...
**Variants**: `Event`, `Function`, `Procedure`, `Table`, `Trigger`, `View`
...
**Variants**: `Like`, `ILike`, `Where`, `NoKeyword`
...
`enum` · `sqlparser::ast::ShowStatementFilterPosition`

```rust
enum ShowStatementFilterPosition
```

**Variants**: `Infix`, `Suffix`
...
`enum` · `sqlparser::ast::ShowStatementInClause`
...
`enum` · `sqlparser::ast::ShowStatementInParentType`

```rust
enum ShowStatementInParentType
```

**Variants**: `Account`, `Database`, `Schema`, `Table`, `View`
...
`enum` · `sqlparser::ast::SqlOption`

```rust
enum SqlOption
```

**Variants**: `Clustered`, `Ident`, `KeyValue`, `Partition`, `Comment`, `TableSpace`, `NamedParenthesizedList`
...
**Variants**: `Rollback`, `Abort`, `Fail`, `Ignore`, `Replace`
...
**Variants**: `Analyze`, `Set`, `Truncate`, `Msck`, `Query`, `Insert`, `Install`, `Load`, `Directory`, `Case`, `If`, `While`, `Raise`, `Call`, `Copy`, `CopyIntoSnowflake`, `Open`, `Close`, `Update`, `Delete`, `CreateView`, `CreateTable`, `CreateVirtualTable`, `CreateIndex`, `CreateRole`, `CreateSecret`, `CreateServer`, `CreatePolicy`, `CreateConnector`, `CreateOperator`, `CreateOperatorFamily`, `CreateOperatorClass`, `AlterTable`, `AlterSchema`, `AlterIndex`, `AlterView`, `AlterFunction`, `AlterType`, `AlterCollation`, `AlterOperator`, `AlterOperatorFamily`, `AlterOperatorClass`, `AlterRole`, `AlterPolicy`, `AlterConnector`, `AlterSession`, `AttachDatabase`, `AttachDuckDBDatabase`, `DetachDuckDBDatabase`, `Drop`, `DropFunction`, `DropDomain`, `DropProcedure`, `DropSecret`, `DropPolicy`, `DropConnector`, `Declare`, `CreateExtension`, `CreateCollation`, `DropExtension`, `DropOperator`, `DropOperatorFamily`, `DropOperatorClass`, `Fetch`, `Flush`, `Discard`, `ShowFunctions`, `ShowVariable`, `ShowStatus`, `ShowVariables`, `ShowCreate`, `ShowColumns`, `ShowCatalogs`, `ShowDatabases`, `ShowProcessList`, `ShowSchemas`, `ShowCharset`, `ShowObjects`, `ShowTables`, `ShowViews`, `ShowCollation`, `Use`, `StartTransaction`, `Comment`, `Commit`, `Rollback`, `CreateSchema`, `CreateDatabase`, `CreateFunction`, `CreateTrigger`, `DropTrigger`, `CreateProcedure`, `CreateMacro`, `CreateStage`, `Assert`, `Grant`, `Deny`, `Revoke`, `Deallocate`, `Execute`, `Prepare`, `Kill`, `ExplainTable`, `Explain`, `Savepoint`, `ReleaseSavepoint`, `Merge`, `Cache`, `UNCache`, `CreateSequence`, `CreateDomain`, `CreateType`, `Pragma`, `Lock`, `LockTables`, `UnlockTables`, `Unload`, `OptimizeTable`, `LISTEN`, `UNLISTEN`, `NOTIFY`, `LoadData`, `RenameTable`, `List`, `Remove`, `RaisError`, `Throw`, `Print`, `WaitFor`, `Return`, `ExportData`, `CreateUser`, `AlterUser`, `Vacuum`, `Reset`
...
`enum` · `sqlparser::ast::StorageSerializationPolicy`

```rust
enum StorageSerializationPolicy
```

**Variants**: `Compatible`, `Optimized`
...
`enum` · `sqlparser::ast::StorageType`

```rust
enum StorageType
```

**Variants**: `Disk`, `Memory`
...
`enum` · `sqlparser::ast::Subscript`

```rust
enum Subscript
```

**Variants**: `Index`, `Slice`
...
`enum` · `sqlparser::ast::TableObject`

```rust
enum TableObject
```

**Variants**: `TableName`, `TableFunction`, `TableQuery`
...
**Variants**: `ColumnstoreIndex`, `ColumnstoreIndexOrder`, `Index`
...
`enum` · `sqlparser::ast::TransactionAccessMode`
...
**Variants**: `AccessMode`, `IsolationLevel`
...
`enum` · `sqlparser::ast::TransactionModifier`

```rust
enum TransactionModifier
```

**Variants**: `Deferred`, `Immediate`, `Exclusive`, `Try`, `Catch`
...
`enum` · `sqlparser::ast::TruncateIdentityOption`

```rust
enum TruncateIdentityOption
```

**Variants**: `Restart`, `Continue`
...
`enum` · `sqlparser::ast::UserPolicyKind`

```rust
enum UserPolicyKind
```

**Variants**: `Authentication`, `Password`, `Session`
...
`enum` · `sqlparser::ast::WaitForType`
...
`enum` · `sqlparser::ast::WindowFrameUnits`
...
`enum` · `sqlparser::ast::WindowType`

```rust
enum WindowType
```

**Variants**: `WindowSpec`, `NamedWindow`
...
`enum` · `sqlparser::ast::WrappedCollection`

```rust
enum WrappedCollection<T>
```

**Variants**: `NoWrapping`, `Parentheses`
...
`struct` · `sqlparser::ast::AlterUser`

```rust
struct AlterUser
```

**Fields**: `if_exists`, `name`, `rename_to`, `reset_password`, `abort_all_queries`, `add_role_delegation`, `remove_role_delegation`, `enroll_mfa`, `set_default_mfa_method`, `remove_mfa_method`, `modify_mfa_method`, `add_mfa_method_otp`, `set_policy`, `unset_policy`, `set_tag`, `unset_tag`, `set_props`, `unset_props`, `password`
...
`struct` · `sqlparser::ast::AlterUserAddMfaMethodOtp`
...
`struct` · `sqlparser::ast::AlterUserAddRoleDelegation`

```rust
struct AlterUserAddRoleDelegation
```

**Fields**: `role`, `integration`
...
`struct` · `sqlparser::ast::AlterUserModifyMfaMethod`

```rust
struct AlterUserModifyMfaMethod
```

**Fields**: `method`, `comment`
...
`struct` · `sqlparser::ast::AlterUserPassword`

```rust
struct AlterUserPassword
```

**Fields**: `encrypted`, `password`
...
`struct` · `sqlparser::ast::AlterUserRemoveRoleDelegation`
...
`struct` · `sqlparser::ast::AlterUserSetPolicy`

```rust
struct AlterUserSetPolicy
```

**Fields**: `policy_kind`, `policy`
...
`struct` · `sqlparser::ast::Analyze`

```rust
struct Analyze
```

**Fields**: `table_name`, `partitions`, `for_columns`, `columns`, `cache_metadata`, `noscan`, `compute_statistics`, `has_table_keyword`
...
`struct` · `sqlparser::ast::Assignment`
...
`struct` · `sqlparser::ast::BeginEndStatements`

```rust
struct BeginEndStatements
```

**Fields**: `begin_token`, `statements`, `end_token`
...
`struct` · `sqlparser::ast::CaseStatement`

```rust
struct CaseStatement
```

**Fields**: `case_token`, `match_expr`, `when_blocks`, `else_block`, `end_case_token`
...
`struct` · `sqlparser::ast::ClusteredIndex`
...
`struct` · `sqlparser::ast::ConditionalStatementBlock`

```rust
struct ConditionalStatementBlock
```

**Fields**: `start_token`, `condition`, `then_token`, `conditional_statements`
...
`struct` · `sqlparser::ast::ContactEntry`

```rust
struct ContactEntry
```

**Fields**: `purpose`, `contact`
...
`struct` · `sqlparser::ast::CreateServerOption`
...
`struct` · `sqlparser::ast::CreateServerStatement`

```rust
struct CreateServerStatement
```

**Fields**: `name`, `if_not_exists`, `server_type`, `version`, `foreign_data_wrapper`, `options`
...
`struct` · `sqlparser::ast::CreateTableLike`
...
`struct` · `sqlparser::ast::CreateUser`

```rust
struct CreateUser
```

**Fields**: `or_replace`, `if_not_exists`, `name`, `options`, `with_tags`, `tags`
...
`struct` · `sqlparser::ast::CreateViewParams`

```rust
struct CreateViewParams
```

**Fields**: `algorithm`, `definer`, `security`
...
`struct` · `sqlparser::ast::Declare`

```rust
struct Declare
```

**Fields**: `names`, `data_type`, `assignment`, `declare_type`, `binary`, `sensitive`, `scroll`, `hold`, `for_query`
...
`struct` · `sqlparser::ast::DenyStatement`
...
`struct` · `sqlparser::ast::DictionaryField`
...
`struct` · `sqlparser::ast::DisplaySeparated`
...
`struct` · `sqlparser::ast::DoUpdate`

```rust
struct DoUpdate
```

**Fields**: `assignments`, `selection`
...
`struct` · `sqlparser::ast::DropDomain`

```rust
struct DropDomain
```

**Fields**: `if_exists`, `name`, `drop_behavior`
...
`struct` · `sqlparser::ast::ExceptionWhen`

```rust
struct ExceptionWhen
```

**Fields**: `idents`, `statements`
...
`struct` · `sqlparser::ast::ExportData`

```rust
struct ExportData
```

**Fields**: `options`, `query`, `connection`
...
`struct` · `sqlparser::ast::Function`

```rust
struct Function
```

**Fields**: `name`, `uses_odbc_syntax`, `parameters`, `args`, `filter`, `null_treatment`, `over`, `within_group`
...
`struct` · `sqlparser::ast::FunctionArgumentList`

```rust
struct FunctionArgumentList
```

**Fields**: `duplicate_treatment`, `args`, `clauses`
...
`struct` · `sqlparser::ast::FunctionDefinitionSetParam`
...
`struct` · `sqlparser::ast::FunctionDesc`
...
`struct` · `sqlparser::ast::Grantee`

```rust
struct Grantee
```

**Fields**: `grantee_type`, `name`
...
`struct` · `sqlparser::ast::HavingBound`
...
**Fields**: `row_format`, `serde_properties`, `storage`, `location`
...
`struct` · `sqlparser::ast::HiveLoadDataFormat`

```rust
struct HiveLoadDataFormat
```

**Fields**: `serde`, `input_format`
...
`struct` · `sqlparser::ast::HiveRowDelimiter`
...
`struct` · `sqlparser::ast::HiveSetLocation`

```rust
struct HiveSetLocation
```

**Fields**: `has_set`, `location`
...
`struct` · `sqlparser::ast::Ident`

```rust
struct Ident
```

**Fields**: `value`, `quote_style`, `span`
...
`struct` · `sqlparser::ast::IfStatement`

```rust
struct IfStatement
```

**Fields**: `if_block`, `elseif_blocks`, `else_block`, `end_token`
...
`struct` · `sqlparser::ast::InsertAliases`

```rust
struct InsertAliases
```

**Fields**: `row_alias`, `col_aliases`
...
`struct` · `sqlparser::ast::Interval`

```rust
struct Interval
```

**Fields**: `value`, `leading_field`, `leading_precision`, `last_field`, `fractional_seconds_precision`
...
`struct` · `sqlparser::ast::JsonReturningClause`
...
`struct` · `sqlparser::ast::LambdaFunction`

```rust
struct LambdaFunction
```

**Fields**: `params`, `body`, `syntax`
...
`struct` · `sqlparser::ast::LambdaFunctionParameter`
...
`struct` · `sqlparser::ast::Lock`

```rust
struct Lock
```

**Fields**: `tables`, `lock_mode`, `nowait`
...
`struct` · `sqlparser::ast::LockTable`

```rust
struct LockTable
```

**Fields**: `table`, `alias`, `lock_type`
...
`struct` · `sqlparser::ast::LockTableTarget`

```rust
struct LockTableTarget
```

**Fields**: `name`, `only`, `has_asterisk`
...
`struct` · `sqlparser::ast::Method`

```rust
struct Method
```

**Fields**: `expr`, `method_chain`
...
`struct` · `sqlparser::ast::NamedParenthesizedList`
...
`struct` · `sqlparser::ast::ObjectNamePartFunction`
...
`struct` · `sqlparser::ast::OneOrManyWithParensIntoIter`
...
`struct` · `sqlparser::ast::OpenStatement`
...
`struct` · `sqlparser::ast::OperateFunctionArg`

```rust
struct OperateFunctionArg
```

**Fields**: `mode`, `name`, `data_type`, `default_expr`
...
`struct` · `sqlparser::ast::OptimizerHint`

```rust
struct OptimizerHint
```

**Fields**: `prefix`, `text`, `style`
...
`struct` · `sqlparser::ast::Parens`

```rust
struct Parens<T>
```

**Fields**: `opening_token`, `content`, `closing_token`

**Implements**: `core::ops::deref::Deref`, `core::ops::deref::DerefMut`, `serde_core::de::Deserialize`, `serde_core::ser::Serialize`, `sqlparser::ast::spans::Spanned`, `sqlparser::ast::visitor::Visit`, `sqlparser::ast::visitor::VisitMut`
...
`struct` · `sqlparser::ast::PrintStatement`
...
`struct` · `sqlparser::ast::RaiseStatement`
...
`struct` · `sqlparser::ast::RenameTable`
...
`struct` · `sqlparser::ast::ResetStatement`
...
`struct` · `sqlparser::ast::ReturnStatement`
...
`struct` · `sqlparser::ast::SecretOption`
...
`struct` · `sqlparser::ast::SetAssignment`

```rust
struct SetAssignment
```

**Fields**: `scope`, `name`, `value`
...
`struct` · `sqlparser::ast::SetSessionAuthorizationParam`
...
`struct` · `sqlparser::ast::SetSessionParamGeneric`
...
`struct` · `sqlparser::ast::SetSessionParamIdentityInsert`
...
`struct` · `sqlparser::ast::SetSessionParamOffsets`
...
`struct` · `sqlparser::ast::SetSessionParamStatistics`
...
`struct` · `sqlparser::ast::ShowCharset`

```rust
struct ShowCharset
```

**Fields**: `is_shorthand`, `filter`
...
`struct` · `sqlparser::ast::ShowObjects`

```rust
struct ShowObjects
```

**Fields**: `terse`, `show_options`
...
`struct` · `sqlparser::ast::ShowStatementIn`

```rust
struct ShowStatementIn
```

**Fields**: `clause`, `parent_type`, `parent_name`
...
`struct` · `sqlparser::ast::ShowStatementOptions`

```rust
struct ShowStatementOptions
```

**Fields**: `show_in`, `starts_with`, `limit`, `limit_from`, `filter_position`
...
`struct` · `sqlparser::ast::StorageLifecyclePolicy`
...
`struct` · `sqlparser::ast::StructField`

```rust
struct StructField
```

**Fields**: `field_name`, `field_type`, `options`
...
`struct` · `sqlparser::ast::TableAliasWithoutColumns`

```rust
struct TableAliasWithoutColumns
```

**Fields**: `explicit`, `alias`
...
`struct` · `sqlparser::ast::TablespaceOption`
...
`struct` · `sqlparser::ast::ThrowStatement`

```rust
struct ThrowStatement
```

**Fields**: `error_number`, `message`, `state`
...
`struct` · `sqlparser::ast::TruncateTableTarget`
...
`struct` · `sqlparser::ast::TypedString`

```rust
struct TypedString
```

**Fields**: `data_type`, `value`, `uses_odbc_syntax`
...
`struct` · `sqlparser::ast::UnionField`
...
`struct` · `sqlparser::ast::UnloadPartitionBy`
...
`struct` · `sqlparser::ast::UtilityOption`
...
`struct` · `sqlparser::ast::VacuumStatement`

```rust
struct VacuumStatement
```

**Fields**: `full`, `sort_only`, `delete_only`, `reindex`, `recluster`, `table_name`, `threshold`, `boost`
...
`struct` · `sqlparser::ast::WaitForStatement`

```rust
struct WaitForStatement
```

**Fields**: `wait_type`, `expr`
...
`struct` · `sqlparser::ast::WhileStatement`
...
**Fields**: `window_name`, `partition_by`, `order_by`, `window_frame`

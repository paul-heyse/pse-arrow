...
def write(root: Path, content: Path) -> dict[str, int]:
    ...
    manifest = json.loads((root / "build/manifests/deltalake.json").read_text())
    packages = {c["package"] for s in manifest["crate_sets"] for c in s["crates"]}
    ...
    if receipt.get("runner_sha256") != digest(root / "scripts/run_probes.py"):
        raise ValueError("Probe runner changed after execution; rerun probes")
    if (
        receipt.get("profile_sha256")
        and digest(root / "skill_improvement/evidence/implementation/runtime-profile.json")
        != receipt["profile_sha256"]
    ):
        raise ValueError("Runtime profile changed after probe execution")
    ...
    for path in sorted((root / "authoring/capabilities").glob("*.json")):
        ...
        for common in [
            "build/manifests/deltalake.json",
            "build/inputs.lock.json",
            "content/index/features.tsv",
        ...
        if record["id"] in {
            ...
            seam = "content/integration/datafusion-arrow-contracts.json"
            dependency_files[seam] = digest(root / seam)
        ...
        for evidence in record["evidence"]:
            if evidence["kind"] == "runtime_observation":
                ...
                dependency_files[runtime_profile] = digest(root / runtime_profile)
                dependency_files.update(receipt.get("sources", {}))
                dependency_files["scripts/run_probes.py"] = digest(root / "scripts/run_probes.py")
    ...
    for record in records:
        ...
        for name in ["lifecycle", "effects", "errors", "configuration", "resources"]:
            if record.get(name):
                lines += ["", f"## {name.title()}", ""] + [f"- {s}" for s in record[name]]
    ...
    for role in roles:
        ...
        crate_lines.append(
            "| "
            + " | ".join(
                [role["crate"], role["role"], role["representations"], role["tasks"], cards]
            )
            + " |"
        )
    ...
    for r in records:
        lines.append(
            f"| [{r['title']}](../capabilities/{r['id']}.md) | "
            f"{'; '.join(r['effects'])} | {'; '.join(r['errors'])} |"
        )
    (routes / "effects.md").write_text("\n".join(lines) + "\n")
    coverage = {

...
class IdentityTests(unittest.TestCase):
    def test_rustdoc_id_renumbering(self) -> None:
        ...
        for key in ["4", "5"]:
            document["index"][key]["span"] = {
                "filename": "/capture-one/.cargo/git/checkouts/dep-abc/abcd123/kernel/src/lib.rs",
                "begin": [10, 1],
                "end": [20, 2],
            }
        located = collect("sample", "1.0.0", document, {"sample::Thing"})
        for key in ["4", "5"]:
            document["index"][key]["span"]["filename"] = (
                "/capture-two/.cargo/git/checkouts/dep-abc/abcd123/kernel/src/lib.rs"
            )
        relocated = collect("sample", "1.0.0", document, {"sample::Thing"})
        self.assertEqual([r["id"] for r in located], [r["id"] for r in relocated])
        for key in ["4", "5"]:
            document["index"][key].pop("span")

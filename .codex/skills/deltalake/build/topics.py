    ...
    for topic in payload["topics"]:
        ...
        if points:
            ...
            for leaf in points:
                ...
                # Name the page from the resolved item, not from the seed: a seed has to be
                # qualified when the leaf name is shared, and the page is still named by leaf.
                page = f"../traits/{item.name}.md"
                exists = content.joinpath("traits", f"{item.name}.md").exists()
                link = f"[{item.name}]({page})" if exists else item.name
                lines.append(

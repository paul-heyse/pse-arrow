...
use deltalake::delta_datafusion::{DeltaCdfTableProvider, session::create_session};
...
async fn e05b_logical_plan_write_preserves_caller_udf_and_runtime() -> TestResult {
    ...
    let table = empty_table().write(Vec::<RecordBatch>::new())
        .with_input_plan(frame.into_optimized_plan()?)
        .with_session_state(Arc::new(state)).await?;
    assert_eq!(table_ids(&table).await, vec![104]);
    Ok(())
}
...
async fn e11b_cdf_preserves_insert_update_delete_and_explicit_versions() -> TestResult {
    ...
    for b in &batches {
        ...
        let kinds = b.column_by_name("_change_type").unwrap().as_any().downcast_ref::<StringArray>().unwrap();
        ...
        for i in 0..b.num_rows() { changes.push((versions.value(i), kinds.value(i).to_string(), values.value(i))); }
    }
    changes.sort();

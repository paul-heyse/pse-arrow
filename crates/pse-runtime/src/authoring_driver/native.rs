...
use crate::authoring_driver::ParseBudget;
use datafusion::{
    ...
    let grouped = LogicalPlanBuilder::from(documents)
        ...
        .project(spec.columns.iter().map(|field| {
            ...
            let value = if field.nullable() {
                ...
                pse_engine::session::scalar::require_nonnull(value)
            };
            value.alias(field.name())
        }))?
        .build()?;
    pse_engine::session::output::declare_relation_output(grouped, registry, &spec)
}

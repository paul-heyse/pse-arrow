...
fn valid_name(name: &str) -> Result<(), DriverError> {
    let expression = crate::authoring_driver::dsl::parse_expr(name)
        .map_err(|error| contract(None, &error.to_string()))?;
    if !matches!(expression.kind, crate::authoring_driver::dsl::ExprKind::Path(ref path) if path.segments.len() == 1 && path.segments[0].name == name && path.segments[0].indices.is_empty())
    {
        return Err(contract(
            None,
...
fn entity(
    ...
    id: SemanticId,
) -> Result<authored::entities::Row, DriverError> {
    let batch = batches
...
fn yaml_scalar(text: &str) -> Result<String, DriverError> {
...
struct TargetResolution<'a> {
    ...
    session: &'a pse_engine::session::EngineSession,
    work: &'a mut pse_columnar::MemoryReservation,
    completed: &'a mut crate::authoring_driver::native_relations::plans::Completions,
        ...
        for section in &document.declaration.sections {
            ...
            for (ordinal, row) in rows.iter().enumerate() {
                for column in &columns {
                    ...
                    let mut path = pse_authoring::targets::parse(text, span)?;
                    let bound = crate::authoring_driver::targets::resolve_native(
                        &path,
                        batches,
                        session,
                        SemanticId::NIL,
...
fn rewrite_target(
    path: &mut pse_authoring::targets::TargetPath,
    bound: &[pse_authoring::targets::TargetRow],
...
fn render_target(path: &pse_authoring::targets::TargetPath) -> String {
        ...
        text.push_str(
            &path
                ...
                .map(|selector| match selector {
                    pse_authoring::targets::IndexSelector::Wildcard => "*".to_owned(),
                    pse_authoring::targets::IndexSelector::Value(value) => value.clone(),
                    pse_authoring::targets::IndexSelector::Label(value) => format!("'{value}'"),
                })
                .collect::<Vec<_>>()
                .join(","),
        );

...
pub(super) fn parse_yaml(
    ...
    document: SemanticId,
    budget: &ParseBudget,
) -> Result<(Spanned<Value>, SpanIndex), DriverError> {
...
fn parse_yaml_value(
    ...
    document: SemanticId,
    budget: &ParseBudget,
) -> Result<Spanned<Value>, DriverError> {
...
pub(super) fn yaml_error(error: &serde_saphyr::Error, document: SemanticId) -> DriverError {
    let start = error
        .location()
        .and_then(|location| location.span().byte_offset())
        .and_then(|offset| u32::try_from(offset).ok())
        .unwrap_or(0);
    DriverError::Authoring(pse_authoring::AuthoringError::Syntax {
...
fn source_span(location: Location, document: SemanticId) -> Result<SourceSpan, DriverError> {
    let span = location.span();
    let (Some(start), Some(length)) = (span.byte_offset(), span.byte_len()) else {
        return Err(DriverError::Authoring(
            pse_authoring::AuthoringError::Syntax {
    ...
    let range = |value| {
        u32::try_from(value).map_err(|_| {
            DriverError::Authoring(pse_authoring::AuthoringError::Budget {
                limit: "source span",
...
fn node_span(node: &Spanned<Value>, document: SemanticId) -> Result<SourceSpan, DriverError> {

...
pub(super) async fn correspondence(
    ...
    let execution = crate::authoring_driver::native_relations::plans::session(session)?;
    for (id, expected) in expected {
        ...
        let bound = crate::authoring_driver::native_relations::plans::roles(
            &execution,
            actual.clone(),
            expected.clone(),
            cancel,
        ...
        for role in ["change_before", "change_after"] {
            completed.push(
                crate::authoring_driver::native_relations::unique(&bound, role, spec, cancel)
                    .await?,
            );
        }
        let plan = crate::authoring_driver::native_relations::plans::difference(&bound, spec)?;

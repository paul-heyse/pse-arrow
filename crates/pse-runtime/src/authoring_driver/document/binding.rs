...
pub use pse_compiler::source_binding::{
    BoundInstancePath, BoundPath, InstancePathSegment, ParsedExpression, PathMeaning,
};
...
pub async fn bind_sources_owned(
...
) -> Result<OwnedSourceBindings, DriverError> {
    bundles.validate_registry(session.registry())?;
    cancel.checkpoint()?;
    let work =
        pse_columnar::MemoryConsumer::new("authoring:source-bindings").register(session.pool());
    let decoded = [
        ...
        decoded
            ...
            .try_fold(0_usize, |sum, batch| {
                crate::authoring_driver::work::add(
                    sum,
                    pse_columnar::algorithm_decode_extent(batch.batch())?,
                )
            })?;
    work.try_grow(crate::authoring_driver::work::add(
        crate::authoring_driver::work::sources(bundles.bundles())?,
        crate::authoring_driver::work::mul(columns, 4)?,
    )?)?;
    ...
    Ok(OwnedSourceBindings(std::sync::Arc::new(BindingOwner {
        bindings,
        _lease: pse_columnar::AllocationLease::new(work),
    })))
}
#[derive(Debug)]
struct BindingOwner {
...
impl OwnedSourceBindings {
    ...
    pub fn completions(&self) -> &[std::sync::Arc<pse_engine::session::CompletedComputation>] {
...
fn bind_parsed(
    ...
    registry: &Registry,
    context: Context,
) -> Result<SourceBindings, DriverError> {
...
) -> Result<ParsedExpression, DriverError> {
...
pub(super) fn parse_document(
    ...
    registry: &Registry,
) -> Result<BTreeMap<String, ParsedExpression>, DriverError> {

...
impl Walker<'_> {
    fn path(&mut self, path: &mut dsl::Path) -> Result<(), DriverError> {
        let binding = self
            .paths
            .next()
    ...
    fn expr(&mut self, expr: &mut dsl::Expr) -> Result<(), DriverError> {
        match &mut expr.kind {
            ExprKind::Number(_) => {}
    ...
    fn predicate(&mut self, predicate: &mut dsl::Predicate) -> Result<(), DriverError> {
    ...
    fn equation(&mut self, equation: &mut dsl::Equation) -> Result<(), DriverError> {

...
pub(super) struct Lookup {
    pub(super) completed: crate::authoring_driver::native_relations::plans::Completions,
...
fn kind(value: &str, registry: &Registry) -> Result<Expr, DriverError> {
    let column = FieldContract::payload(
        "kind",
        FieldContract::enumeration("SourceBindingKind"),
        "Binding kind.",
    );
    pse_engine::session::output::checked_literal(
        registry,
        &column,
...
fn declarations(session: &EngineSession, inputs: &Batches) -> Result<LogicalPlan, DriverError> {
    let mut result = None;
...
) -> Result<BTreeMap<&'static str, datafusion::arrow::array::ArrayRef>, DriverError> {
    ["field_path", "kind", "name"]
        .into_iter()
        .map(|name| {
            let array = batch
                .column_by_name(name)
        ...
        .collect::<Result<BTreeMap<_, _>, DriverError>>()
}
async fn consume(
    session: &EngineSession,
    plan: LogicalPlan,
    ...
    for batch in crate::authoring_driver::native_relations::plans::execute_recorded(
        session,
        plan,
        cancel,
    ...
    .await?
    {
        let strings = binding_strings(&batch)?;
        for row in 0..batch.num_rows() {
            let id = |name: &str| -> Result<Option<SemanticId>, DriverError> {
                let array = batch
                    .column_by_name(name)
            ...
            let text = |name: &str| -> Result<String, DriverError> {
                let array = strings
                    .get(name)
                    .ok_or_else(|| contract(None, "binding text absent"))?;
                Ok(array
    ...
    output
        .resolved
        .push(builder.finish()?.retained(session.pool(), cancel)?);
    Ok(())
}
            ...
            for document in &bundle.documents {
                for section in &document.declaration.sections {
                    ...
                    for (row_ordinal, value) in values.iter().enumerate() {
                        ...
                        let owner_id = crate::authoring_driver::ids::parse_id(
                            owner,
                            crate::authoring_driver::SourceSpan::head(document.id),
                        )?;
...
fn occurrence_rows(
    document: &crate::authoring_driver::document::Document,
    ...
    let batch = builder.finish()?.retained(session.pool(), cancel)?;
    Ok(batch)
}

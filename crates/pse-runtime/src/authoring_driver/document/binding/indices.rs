...
) -> Result<IndexCompanions, DriverError> {
...
fn string_list(array: &ListArray, row: usize) -> Result<Vec<String>, DriverError> {
    let values = array.value(row);
    let values =
        datafusion::arrow::compute::cast(&values, &datafusion::arrow::datatypes::DataType::Utf8)
            .map_err(|error| {
                crate::authoring_driver::native_relations::plans::engine(error.into())
            })?;
    let values = values
        .as_any()
...
fn companion_join(
...
) -> Result<datafusion::logical_expr::LogicalPlan, DriverError> {
    ...
    let source_plan = LogicalPlanBuilder::from(bound.scan_role("index_source")?)
        ...
        .map_err(crate::authoring_driver::native_relations::plans::engine)?;

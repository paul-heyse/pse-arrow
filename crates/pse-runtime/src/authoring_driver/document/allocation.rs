...
impl<'a> Allocation<'a> {
    pub(super) fn new(pool: &'a Arc<dyn MemoryPool>, cancel: &'a CancellationToken) -> Self {
        Self {
            cancel,
            reservation: pse_columnar::MemoryConsumer::new("authoring:owned-documents")
                .register(pool),
        }
    }
    pub(super) fn grow(&mut self, bytes: usize) -> Result<(), DriverError> {
    ...
    pub(super) fn retain(&mut self, start: usize, bytes: usize) -> Result<(), DriverError> {
    ...
    pub(super) fn finish(self) -> Arc<AllocationLease> {
        AllocationLease::new(self.reservation)
    }
}

pub(super) fn add(a: usize, b: usize) -> Result<usize, DriverError> {
    a.checked_add(b).ok_or(DriverError::Authoring(
...
pub(super) fn mul(a: usize, b: usize) -> Result<usize, DriverError> {
    a.checked_mul(b).ok_or(DriverError::Authoring(
        pse_authoring::AuthoringError::Budget {
            limit: "allocation extent",
            allowed: usize::MAX as u64,
            needed: u64::MAX,
...
fn lexical_nodes(text: &str) -> Result<usize, DriverError> {
    let mut tokens = 0;
...
pub(super) fn yaml_node_limit(text: &str, budget: &ParseBudget) -> Result<usize, DriverError> {
...
pub(super) fn value_retained(value: &Value) -> Result<usize, DriverError> {
...
pub(super) fn visit_rows(
    document: &Document,
    mut visit: impl FnMut(&DocumentSection, &Value) -> Result<(), DriverError>,
) -> Result<(), DriverError> {
...
pub(super) fn row_extent(document: &Document, registry: &Registry) -> Result<usize, DriverError> {
...
fn field_extent(value: Option<&Value>, ty: &FieldContract) -> Result<usize, DriverError> {
...
fn expression_extent(value: &Value, ty: &FieldContract, path: usize) -> Result<usize, DriverError> {
...
pub(super) fn registry_extent(registry: &Registry) -> Result<usize, DriverError> {
    let mut bytes = mul(
...
mod tests {
    ...
    fn path_preflight_accounts_complete_escaped_ancestor_paths() {
        ...
        let shared: Arc<dyn MemoryPool> =
            Arc::new(pse_columnar::GreedyMemoryPool::new(parser + paths - 1));
        let cancel = CancellationToken::new();
        {
            let mut allocation = Allocation::new(&shared, &cancel);

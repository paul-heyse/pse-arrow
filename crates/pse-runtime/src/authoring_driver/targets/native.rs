...
use crate::authoring_driver::{DriverError, document::Batches};
...
fn contract(at: Option<crate::authoring_driver::SourceSpan>, reason: &str) -> DriverError {
    DriverError::Authoring(pse_authoring::AuthoringError::Contract {
        at,
        reason: reason.to_owned(),
    })
}
fn engine(error: datafusion::common::DataFusionError) -> DriverError {
    crate::authoring_driver::native_relations::plans::engine(error)
}
fn field(alias: &str, name: &str) -> Expr {
    Expr::Column(Column::new(Some(alias), name))
}
fn identity(session: &EngineSession, id: SemanticId) -> Result<Expr, DriverError> {
    checked_literal(
        session.registry(),
...
fn scan(session: &EngineSession, id: SemanticId) -> Result<LogicalPlan, DriverError> {
...
) -> Result<FieldCheckedBatch, DriverError> {
...
async fn materialize(
    ...
    let batches = crate::authoring_driver::native_relations::plans::execute_recorded(
        session, plan, cancel, completed,
    )
    .await?;
    let work = pse_columnar::MemoryConsumer::new("authoring:target-native-output")
        .register(session.pool());
    work.try_grow(batches.iter().try_fold(0_usize, |sum, batch| {
        crate::authoring_driver::work::add(sum, pse_columnar::algorithm_decode_extent(batch)?)
    })?)?;
...
pub async fn resolve_native(
    ...
    completed: &mut Vec<std::sync::Arc<pse_engine::session::CompletedComputation>>,
    cancel: &CancellationToken,
) -> Result<Vec<TargetRow>, DriverError> {
    ...
    let session = crate::authoring_driver::native_relations::plans::session(&session)?;
    ...
    let tuples = crate::authoring_driver::native_relations::plans::execute_recorded(
        &session, plan, cancel, completed,
    )
    .await?;
    ...
    work.try_grow(crate::authoring_driver::work::mul(
        count,
        crate::authoring_driver::work::add(
            size_of::<TargetRow>(),
            crate::authoring_driver::work::mul(axes.len(), size_of::<SemanticId>())?,
        )?,
    )?)?;
...
async fn instance(
    ...
    cancel: &CancellationToken,
    completed: &mut crate::authoring_driver::native_relations::plans::Completions,
) -> Result<authored::instances::Row, DriverError> {
...
async fn domain(
    ...
    cancel: &CancellationToken,
    completed: &mut crate::authoring_driver::native_relations::plans::Completions,
) -> Result<authored::domains::Row, DriverError> {
...
fn selector_condition(
    ...
    if !quoted {
        ...
        if let Some(number) = text.parse::<f64>().ok().filter(|number| number.is_finite()) {
            ...
            .map_err(engine)?;
            condition = condition.or(crate::authoring_driver::native_relations::exact::equal(
                col("coordinate"),
                value,
            ));
        }
...
async fn axis_members(
    ...
    let duplicate = LogicalPlanBuilder::from(plan.clone())
        ...
        .and_then(LogicalPlanBuilder::build)
        .map_err(engine)?;
    if crate::authoring_driver::native_relations::plans::execute_recorded(
        session, duplicate, cancel, completed,
    )
    .await?
    .iter()
    ...
    let selected = crate::authoring_driver::native_relations::plans::execute_recorded(
        session,
        plan.clone(),
        cancel,
        completed,

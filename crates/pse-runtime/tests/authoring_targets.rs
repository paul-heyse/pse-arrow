...
fn context() -> pse_runtime::authoring_driver::document::Batches {
    ...
    let mut rows = pse_relations::columnar::Collection::new(&registry, &budget, &cancel);
...
async fn resolve(
    path: &TargetPath,
    context: &pse_runtime::authoring_driver::document::Batches,
    source: SemanticId,
) -> Result<Vec<TargetRow>, pse_runtime::authoring_driver::DriverError> {
...
async fn resolve_cancel(
    ...
    let mut work = pse_columnar::MemoryConsumer::new("target-fixture").register(session.pool());
    pse_runtime::authoring_driver::targets::resolve_native(
        path,
        context,
        &session,
        source,
...
fn change<T: pse_relations::columnar::RelationRow>(
    batches: &mut pse_runtime::authoring_driver::document::Batches,

...
fn explicit_and_named_fixtures_decode_generated_rows_and_original_source_spans() {
    ...
    .unwrap();
    let species =
        authored::species::View::from_checked(&explicit.batches[&authored::species::RELATION_ID])
            .unwrap()
            .row(0)
            .unwrap();
...
fn nested_named_identities_use_the_actual_declared_parent_independent_of_file_order() {
    ...
    let entities =
        authored::entities::View::from_checked(&bundle.batches[&authored::entities::RELATION_ID])
            .unwrap()
            .rows()
            .unwrap();
    let child = entities
...
async fn declared_field_grammar_preserves_atomic_guards_and_refuses_equations_in_values() {
    ...
    let owned = pse_runtime::authoring_driver::document::load_bundles_owned(
        &[bundle],
        registry,
        &budget,
        &cancel,
    ...
    .unwrap();
    let batches =
        pse_runtime::authoring_driver::p1::source_batches(owned.bundles(), registry).unwrap();
    let bound = pse_runtime::authoring_driver::document::binding::bind_sources_owned(
        &owned, &batches, &session, &cancel,
    )
    .await
    .unwrap();
    assert!(bound.expressions().iter().any(|source| matches!(
        source.parsed,
        pse_runtime::authoring_driver::document::binding::ParsedExpression::Predicate(_)
    )));
    inputs.insert(
...
async fn enum_predicates_bind_exact_known_members_and_explicit_deferred_types() {
    use pse_runtime::authoring_driver::document::binding::{PathMeaning, bind_sources_owned};
...
"
    ...
    for (predicate, admitted) in [
        ...
        .unwrap();
        let batches =
            pse_runtime::authoring_driver::p1::source_batches(documents.bundles(), &registry)
                .unwrap();

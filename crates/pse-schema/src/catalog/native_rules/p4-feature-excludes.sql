...
q10 AS (
    ...
    WHERE (((((((("declarations"."rule" = 'excludes'))))))) AND ((NOT (get_field(get_field("left_feature"."value", 'boolean'), 'value') AND get_field(get_field("right_feature"."value", 'boolean'), 'value')))))
)
SELECT

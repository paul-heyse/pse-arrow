...
fn string_enums_keep_their_domain_in_execution_and_durable_fields() {
    ...
    let compiled = pse_schema::compiled_contract::relation(&registry, relation)
        .unwrap()
        .literal_spec();
    assert!(compiled.contains("unbounded"));
    assert!(!compiled.contains("Dictionary"));
}
...
fn bounded_signed_domains_project_to_languages_and_durable_storage() {
    ...
    assert!(rust.contains("value outside declared integer domain"));
    let python = generate(&registry, Language::Python).unwrap();

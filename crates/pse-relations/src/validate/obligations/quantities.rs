...
pub(super) fn plans(
    ...
    let types = super::selected("reference.quantity_types", inputs, registry);
    let units = super::selected("reference.units", inputs, registry);
    occurrences
        .into_iter()
        .map(|occurrence| {
            let value = occurrence.value()?;
            let values = super::nested_values::append(
                occurrence.input,
                vec![

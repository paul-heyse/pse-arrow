...
//! Mathematical wire expectations, independent of the native scalar adapter.
...
fn dictionary_codes_and_slices_encode_actual_meanings() {
    ...
    let mut text = to_json(&dictionary, &field, 0).unwrap();
    assert_eq!(text, "[\"text\",\"liquid\"]");
    text = to_json(&dictionary, &field, 1).unwrap();
    assert_eq!(text, "[\"text\",\"vapor\"]");
}

#[test]
fn visible_parent_nulls_mask_nested_payload_and_list_offsets_are_exact() {
    let list = ListArray::from_iter_primitive::<UInt64Type, _, _>([
        Some(vec![Some(5), Some(u64::MAX)]),
        Some(vec![Some(8)]),
    ]);
    let list = list.slice(1, 1);
    let field = Field::new("items", list.data_type().clone(), false);
    let mut text = to_json(&list, &field, 0).unwrap();
    ...
    text = to_json(&nested, &field, 0).unwrap();
    assert_eq!(text, "[\"null\",null]");
    text = to_json(&nested, &field, 1).unwrap();

...
//! ordinals and one [`CanonicalField`] per column. No registry type appears here, because
//! `pse-ids` sits below `pse-schema` in the graph and identity must not need the registry
//! to exist.
//!
//! The point of [`CanonicalField`] is the rule in §5.3 step 2: *only registered logical types and
...
/// Checked native field and the resolved domain facts needed by canonicalization.
/// Native `DataType` remains the sole storage description.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct CanonicalField {
    pub(crate) field: Arc<Field>,
    pub(crate) domain: Option<Arc<[String]>>,
    pub(crate) children: Vec<Self>,
}
impl CanonicalField {
    /// Exact native field, including metadata and child declarations.
    pub fn field(&self) -> &Field {
        &self.field
    }
    pub(crate) fn is_floating(&self) -> bool {
        self.field.data_type().is_floating() || self.children.iter().any(Self::is_floating)
    }
    pub(crate) fn dictionary_key(&self) -> Option<&DataType> {
        match self.field.data_type() {
            DataType::Dictionary(key, _) => Some(key.as_ref()),
            _ => None,
        }
    }
}
...
/// Check the native storage matrix and bind the declared enum domains once.
fn layout_of(
    path: &FieldPath,
    field: &Field,
    enum_domains: &BTreeMap<FieldPath, Arc<[String]>>,
) -> Result<CanonicalField, CanonError> {
    ...
    match field.data_type() {
        ...
        DataType::Dictionary(key, value) => {
            ...
            if !enum_domains.contains_key(path) {
        ...
        DataType::List(child) | DataType::FixedSizeList(child, _) => {
            children.push(layout_of(&path.child(0), child, enum_domains)?);
        }
        DataType::Struct(fields) => {
            for (ordinal, child) in fields.iter().enumerate() {
                children.push(layout_of(&path.child(ordinal), child, enum_domains)?);
            }
        }
        _ => return Err(unsupported()),
    }
    let domain = enum_domains.get(path).cloned();
    if domain.is_some() && !matches!(field.data_type(), DataType::Utf8 | DataType::Dictionary(..)) {
        return Err(unsupported());
    }
    Ok(CanonicalField {
...
pub struct CanonicalContract {
    ...
    pub layouts: Vec<CanonicalField>,
}
...
impl CanonicalContract {
    ...
    ///
    /// Unknown metadata is retained as meaning; it is never stripped.
    ...
    pub fn try_new(
        ...
        for name in primary_key {
            ...
            if layouts
                .get(ordinal)
                .is_some_and(CanonicalField::is_floating)
            {
                return Err(CanonError::InvalidKey {
                    column: (*name).to_owned(),
...
mod consolidation_unit {
    ...
    fn every_supported_scalar_layout_is_admitted() {
        ...
        for data_type in cases {
            ...
            match contract {
                Ok(contract) => assert_eq!(
                    contract
                        .layouts
                        .get(1)
                        .map(|field| field.field().data_type()),
                    Some(&data_type)
                ),
    ...
    fn nested_struct_of_list_of_utf8_is_admitted() {
        ...
        assert_eq!(
            contract
                .ok()
                .as_ref()
                .and_then(|contract| contract.layouts.get(1))
                .map(CanonicalField::field),
            Some(&expected)
        );
    }
    ...
    fn a_fixed_size_list_keeps_its_width() {
        ...
        let contract = contract_of(vec![key_field(), field.clone()], &["id"], &no_domains());
        assert_eq!(
            contract
                .ok()
                .as_ref()
    ...
    #[test]
    fn a_dictionary_retains_native_storage_and_a_separate_resolved_domain() {
        let members: Arc<[String]> = Arc::from(vec!["liquid".into(), "vapor".into()]);
        ...
        let contract = contract_of(vec![key_field(), field.clone()], &["id"], &domains).unwrap();
        let descriptor = &contract.layouts[1];
        assert_eq!(descriptor.field(), &field);
        assert_eq!(descriptor.domain, Some(members));
        assert_eq!(descriptor.dictionary_key(), Some(&DataType::Int8));
    }
    ...
    #[test]
    fn unknown_metadata_is_retained_and_compared_exactly() {
        let tagged =
            key_field().with_metadata(HashMap::from([("unknown.meaning".into(), "x".into())]));
        let contract = contract_of(vec![tagged.clone()], &["id"], &no_domains()).unwrap();
        assert_eq!(contract.schema.field(0), &tagged);
        assert!(
            contract
                .check_batch_schema(&Schema::new(vec![key_field()]))
                .is_err()
        );
    }

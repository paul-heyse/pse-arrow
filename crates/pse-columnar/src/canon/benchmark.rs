...
use crate::{CanonError, CanonicalContract, CanonicalOutput, CanonicalizeOptions};
...
pub fn canonicalize_stages(
    ...
    batches: &[RecordBatch],
    pool: &std::sync::Arc<dyn crate::MemoryPool>,
    options: CanonicalizeOptions,
) -> Result<(CanonicalOutput, StageTimings), CanonError> {
    let mut timings = StageTimings::default();
    let mut start = Instant::now();
    let result = canonicalize_observed(contract, batches, pool, options, |stage, entering| {
        if entering {
            start = Instant::now();
        } else {

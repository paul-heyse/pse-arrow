...
impl<E: std::fmt::Debug + Send + Sync + 'static> miette::Diagnostic for FlightError<E> {
    fn code<'a>(&'a self) -> Option<Box<dyn std::fmt::Display + 'a>> {
        Some(Box::new(match self {
            ...
            Self::Retiring => "runtime::resource_limit",
...
type SharedLoad<V, E> = Shared<BoxFuture<'static, Result<Arc<V>, FlightError<E>>>>;
struct Flight<V, E> {
    ...
    E: Send + Sync + 'static,
{
    ...
    ) -> Result<Arc<V>, FlightError<E>>
    where
        F: Future<Output = Result<Arc<V>, E>> + Send + 'static,
    {
        let waiter = {
            ...
            if let Some(f) = entries.get(&key) {
                if f.cancel.flag.load(Ordering::Acquire) {
                    return Err(FlightError::Retiring);
                }
                match f
                    .waiters
                    ...
                    Ok(_) => {}
                    Err(0) => return Err(FlightError::Retiring),
                    Err(_) => return Err(FlightError::Capacity),
                }
                Waiter(f.clone())
            } else {

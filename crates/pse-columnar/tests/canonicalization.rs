...
use pse_columnar::{
    CancellationToken, CanonError, CanonicalContract, CanonicalizeOptions, Envelope, FieldPath,
...
fn explicit_singleton_canonicalization_preserves_zero_and_one_row() {
    ...
    let zero = canonicalize(&contract, &[batch(Vec::<i32>::new())], &budget, options()).unwrap();
    let one = canonicalize(&contract, &[batch(vec![7])], &budget, options()).unwrap();
...
fn row_order_batch_boundaries_null_payload_and_dictionary_codes_have_no_identity() {
    ...
    let expected =
        canonicalize(&contract, std::slice::from_ref(&source), &budget, options()).unwrap();
    ...
    let actual = canonicalize(&contract, &batches, &budget, options()).unwrap();
...
fn frame_contains_two_finished_single_batch_streams_with_canonical_hidden_values() {
    ...
    let budget: Arc<dyn pse_columnar::MemoryPool> =
        Arc::new(pse_columnar::GreedyMemoryPool::new(64 << 20));
    let result = canonicalize(&contract, &[source], &budget, options()).unwrap();
...
fn retained_preimage_and_individual_arrow_buffers_keep_the_reservation() {
    ...
    let budget: Arc<dyn pse_columnar::MemoryPool> =
        Arc::new(pse_columnar::GreedyMemoryPool::new(64 << 20));
    let output = canonicalize(&contract, &[source], &budget, options()).unwrap();
    ...
    assert!(
        retained > 0 && retained < reserved,
        "unretained result allocations must release"
    );
    drop(bytes);
    let buffer_claim = budget.reserved();
    assert!(
        buffer_claim > 0 && buffer_claim < retained,
        "the preimage has a separate owner"
    );
    assert!(buffer_claim >= buffer.len());
    drop(buffer);
    assert_eq!(budget.reserved(), 0);
}
...
fn budget_envelope_cancellation_and_duplicate_keys_reject_without_retained_claims() {
    ...
    assert!(matches!(
        logical_hash(&contract, std::slice::from_ref(&source), &tiny),
    ...
    assert_eq!(budget.reserved(), 0);
    assert!(matches!(
        logical_hash(&contract, &[source.clone(), source], &budget),
        Err(CanonError::DuplicateKey { .. })
    ));
    assert_eq!(budget.reserved(), 0);
}
...
fn signed_zero_differs_and_matching_contract_metadata_cannot_admit_invalid_enum() {
    ...
    assert_ne!(
        logical_hash(&contract, std::slice::from_ref(&source), &budget).unwrap(),
        logical_hash(&contract, &[changed], &budget).unwrap()
    );
    columns[2] = Arc::new(
        DictionaryArray::<Int8Type>::try_new(
            Int8Array::from(vec![Some(0), None, Some(1)]),
    ...
    assert!(matches!(
        logical_hash(&contract, &[invalid], &budget),
...
fn empty_relations_and_explicit_all_valid_bitmaps_are_canonical() {
    ...
    let budget: Arc<dyn pse_columnar::MemoryPool> =
        Arc::new(pse_columnar::GreedyMemoryPool::new(64 << 20));
    let empty = canonicalize(&contract, &[], &budget, options()).unwrap();
    assert_eq!(empty.row_count, 0);
    assert_eq!(decoded_data(empty.preimage.as_ref().unwrap()).num_rows(), 0);
    assert_eq!(
        empty.logical_hash,
        logical_hash(&contract, &[source.slice(0, 0)], &budget).unwrap()
    );
    let mut columns = source.columns().to_vec();
    ...
    assert_eq!(
        logical_hash(&contract, &[source], &budget).unwrap(),
        logical_hash(&contract, &[explicit], &budget).unwrap()
    );
}
...
fn fixed_list_descendants_are_forced_valid_zero_under_masked_parents() {
    ...
    let budget: Arc<dyn pse_columnar::MemoryPool> =
        Arc::new(pse_columnar::GreedyMemoryPool::new(64 << 20));
    let output = canonicalize(&contract, &[make(None)], &budget, options()).unwrap();
    assert_eq!(
        output.logical_hash,
        logical_hash(&contract, &[make(Some(999))], &budget).unwrap()
    );
...
fn supported_scalar_storage_and_sliced_input_round_trip_through_canonical_ipc() {
    ...
    let budget: Arc<dyn pse_columnar::MemoryPool> =
        Arc::new(pse_columnar::GreedyMemoryPool::new(64 << 20));
    let output =
        canonicalize(&contract, std::slice::from_ref(&sliced), &budget, options()).unwrap();
    let normalized = decoded_data(output.preimage.as_ref().unwrap());

...
use pse_columnar::{CanonicalContract, CanonicalizeOptions};
...
proptest! {
    ...
    fn arbitrary_null_payloads_and_batch_boundaries_do_not_enter_the_preimage(
        ...
        let budget: Arc<dyn pse_columnar::MemoryPool> = Arc::new(pse_columnar::GreedyMemoryPool::new(16 << 20));
        ...
        let expected = pse_columnar::canonicalize(&contract, &[one], &budget, options.clone()).expect("admitted");
        let actual = pse_columnar::canonicalize(&contract,
            &[other.slice(boundary, values.len() - boundary), other.slice(0, boundary)],
            &budget, options).expect("admitted alternate");
...
fn instrumentation_keeps_the_production_admission_and_reservation_boundary() {
    ...
    let (observed, timings) = pse_columnar::canon::benchmark::canonicalize_stages(
        &contract,
    ...
    let ordinary = pse_columnar::canonicalize(
        &contract,
        std::slice::from_ref(&source),
        &budget,
        options.clone(),
    ...
    assert!(
        pse_columnar::canon::benchmark::canonicalize_stages(
            &contract,
            &[source],
            &{

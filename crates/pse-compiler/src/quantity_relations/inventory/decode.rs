...
) -> Result<(QuantityRegistry, Option<QuantityKindId>), CompilerError> {
...
fn enumeration<T>(name: &str, parse: fn(&str) -> Option<T>) -> Result<T, CompilerError> {
    parse(name).ok_or_else(|| invalid(format!("physical algorithm has no declared {name} member")))
}

fn dimension(value: extension_values::DimensionVector) -> Result<DimensionVector, CompilerError> {

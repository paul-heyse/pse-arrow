...
//! Versioned key tokens over the pinned native Arrow row encoding. Keys remain
//! scoped by their relation and selected input; tokens are not membership proofs.

use datafusion::{
    arrow::{
        array::{ArrayRef, builder::FixedSizeBinaryBuilder},
...
pub fn key(relation: pse_ids::SemanticId, columns: Vec<(&str, Expr)>) -> Expr {
    FUNCTION.call(
        std::iter::once(lit(datafusion::common::ScalarValue::FixedSizeBinary(
            16,
...
impl ScalarUDFImpl for Key {
    ...
    fn return_field_from_args(&self, args: ReturnFieldArgs<'_>) -> Result<FieldRef> {
        ...
        if !matches!(args.scalar_arguments.first().copied().flatten(),
            Some(datafusion::common::ScalarValue::FixedSizeBinary(16, Some(scope))) if scope.len() == 16)
        {
        ...
        for (i, pair) in args.arg_fields[1..].as_chunks::<2>().0.iter().enumerate() {
            ...
            if !RowConverter::supports_fields(&[SortField::new(pair[1].data_type().clone())]) {
                return Err(invalid("key value has no native row encoding"));
            }
        }
        output_field()
    }
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        let mut prefix = pse_ids::FramedHasher::new(pse_schema::model::ROW_KEY_ENCODING);
        let Some(ColumnarValue::Scalar(datafusion::common::ScalarValue::FixedSizeBinary(
            16,
            Some(scope),
        ))) = args.args.first()
        else {
            ...
            .zip(args.arg_fields[1..].as_chunks::<2>().0)
        {
            let ColumnarValue::Scalar(name) = &pair[0] else {
                return Err(invalid("key name is not a literal"));
            };
            prefix.str(
                name.try_as_str()
                    .flatten()
            ...
            // Physical type and meaningful domain identity are part of the key contract.
            // Roles, references, names and documentation describe use, not key values.
            let mut field = declared[1]
                .as_ref()
                .clone()
                .with_name("item")
                .with_nullable(false);
            field.metadata_mut().retain(|name, _| {
                name.starts_with("ARROW:extension:")
                    || name == "pse.domain.extension"
                    || name == "pse.domain.parameter"
                    || name == pse_schema::arrow::KEY_ROW_KEY_ENCODING
            });
            prefix.str(
                &FieldContract::from_field(field)
                    .canonical_json()
                    .map_err(external)?,
            );
            fields.push(SortField::new(declared[1].data_type().clone()));
            arrays.push(pair[1].clone().into_array(args.number_rows)?);
        }
        let converter = RowConverter::new(fields)?;
        let rows = converter.convert_columns(&arrays)?;
        let mut output = FixedSizeBinaryBuilder::with_capacity(args.number_rows, 32);
        for row in 0..args.number_rows {
            let mut hasher = prefix.clone();
            // A declared singleton has one empty tuple even when Arrow has no columns.
            if arrays.is_empty() {
                hasher.part(&[]);
            } else {
                hasher.part(rows.row(row).as_ref());
            }
            output.append_value(hasher.finish_hash().as_bytes())?;
        }

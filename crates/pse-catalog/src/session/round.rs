...
impl SnapshotSession {
    ...
    pub fn with_round_inputs(
    ...
    ) -> std::result::Result<(Self, RoundInputs), CatalogError> {
            ...
            }))
        {
            ...
            let input = input.retained(session.reserver.as_ref(), cancel)?;
...
impl TableProvider for RoundTable {
    ...
    async fn scan(
        ...
        let schema = projection.map_or_else(
            ...
            |p| self.schema.project(p).map(Arc::new),
        )?;
        Ok(Arc::new(RoundExec {
...
impl DisplayAs for RoundExec {
    fn fmt_as(&self, _: DisplayFormatType, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "RoundSourceExec: {}, statistics=unknown", self.name)
    }
}
...
impl ExecutionPlan for RoundExec {
    ...
    fn with_new_children(
        ...
        if !children.is_empty() {
            return Err(invalid("round source has no children"));
        }
        Ok(self)
    }
    fn execute(
        ...
        if partition != 0 {
            return Err(invalid("round source has one partition"));
        }
        let (input, reader) = {
            let mut epoch = self
                .inputs
            ...
            let input = epoch
                ...
                .ok_or_else(|| invalid("round source absent"))?;
            epoch.readers += 1;
            (input, Reader(self.inputs.clone()))
        };
        let batch = super::query_schema::batch(input.batch())?;
        let native = MemorySourceConfig::try_new_exec(
            &[vec![batch.clone()]],
            batch.schema(),
            self.projection.clone(),
        )?;
        let stream = native.execute(0, context)?;
        Ok(Box::pin(RecordBatchStreamAdapter::new(
            self.schema(),
            stream.map_ok(move |batch| {
                let _owners = (&reader, &input);
                batch
            }),
        )))
    }
...
pub(super) fn validate_reset_plan(plan: &Arc<dyn ExecutionPlan>) -> Result<()> {
    ...
    let supported = plan.is::<RoundExec>()
        ...
        || crate::delta::leased::supports_round_reset(plan.as_ref())
        || crate::cache_service::resident::supports_round_reset(plan.as_ref())
        || plan.is::<super::contract::ContractExec>()
        || super::cache::supports_physical_reset(plan.as_ref())
        || plan.is::<datafusion::datasource::source::DataSourceExec>();
    if !supported {
...
mod tests {
    ...
    async fn prepared_joins_observe_empty_then_nonempty_epochs_without_replanning() {
            ...
            .flat_map(|hash| [false, true].map(|cached| (hash, cached)))
        {
            ...
            let session = super::super::SessionFactory::new(

...
pub(super) fn properties(schema: &Schema) -> Result<BTreeMap<String, String>> {
...
pub(super) fn expressions(schema: &Schema, state: &SessionState) -> Result<Vec<Expr>> {
...
mod tests {
    ...
    fn row_checks_bind_boolean_nonvolatile_expressions_in_the_actual_session() {
        ...
        for (sql, valid) in [
            ...
            assert_eq!(expressions(&schema, &state).is_ok(), valid, "{sql}");
        }
    }
}

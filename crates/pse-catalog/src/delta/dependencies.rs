...
pub fn with_member_dependencies(
    ...
    cancel.checkpoint()?;
    if role.is_empty() || source.bindings().input(role).is_some() {
        return Err(pse_engine::session::engine(invalid(
            "dependency role is empty or already bound",
        )));
    }
    let member = crate::selection::selected_member(source, reference)?;
    let schema = native_dependencies::schema()?;
    let partition = Dependencies {
        schema: Arc::clone(&schema),
        member,
        state: Arc::new(source.bound_state()?),
        leases: source.owners().to_vec(),
    };
    ...
    session
        .bind_source(
            ...
            TableBinding::new(
                reference,
                provider,
                Some(native_dependencies::spec(source.registry())?.key),
                None,
            ),
        )
        .map_err(pse_engine::session::engine)?;
...
struct Dependencies {
    ...
    leases: Vec<Arc<dyn pse_engine::provider::witness::ExecutionOwner>>,
}
impl PartitionStream for Dependencies {
    ...
    fn execute(&self, context: Arc<TaskContext>) -> SendableRecordBatchStream {
        ...
        let stream = futures_util::stream::once(async move {
            ...
            Ok(
                pse_columnar::owned_buffer::OwnedRecordBatch::export(
                    batch,
                    services.pool(),
                    cancel,
                )

...
use pse_schema::model::{
    Authority, Cell, ColumnSpec, ExtensionUse, LogicalType, Namespace, RelationDecl, SnapshotClass,
};
...
fn registry_of_class(invariant: bool, class: SnapshotClass) -> Arc<Registry> {
    ...
    builder.declare_relation(
        ...
        .columns(vec![
            ...
            ColumnSpec::payload("value", LogicalType::Text, "value"),
        ]),
    );
    if invariant {
        use pse_schema::model::{
        ...
        builder.declare_rule(RuleDecl::new(
            ...
            RulePlan::Project {
                ...
                columns: vec![("id", RuleExpr::col("input.id"))],
            },
        ));
...
fn document_registry() -> Arc<Registry> {
    ...
    for (name, columns) in [
        (
            "documents",
            vec![
                ColumnSpec::key("document_id", LogicalType::id(), "identity"),
                ColumnSpec::payload(
                    "content_hash",
...
fn document_draft(reg: &Registry, source: &[u8], end: u64) -> BundleDraft {
    ...
    for (name, value) in [
        (
            "authored.documents",
            Cell::Hash(pse_ids::encoding_checksum(source).content_hash()),
        ),
        (
            "authored.spans",
...
fn stage_registry() -> Arc<Registry> {
    ...
    for name in ["items", "other"] {
        builder.declare_relation(
            ...
            .pk(&["id"])
            .columns(vec![ColumnSpec::key("id", LogicalType::U64, "identity")]),
        );
    }
    for (namespace, name, key, columns) in [
        (
            ...
            "model_revision_id",
            vec![
                ColumnSpec::key("model_revision_id", LogicalType::id(), "revision"),
                ColumnSpec::payload("snapshot_id", LogicalType::hash(), "target"),
            ],
        ),
        (
            ...
            "pass_run_id",
            vec![
                ColumnSpec::key("pass_run_id", LogicalType::id(), "attempt"),
                ColumnSpec::payload("pass_id", LogicalType::id(), "producer"),
                ColumnSpec::payload("version", LogicalType::Text, "version"),
                ColumnSpec::payload("snapshot_out", LogicalType::hash(), "target"),
                ColumnSpec::payload("status", LogicalType::Text, "outcome"),
            ],
        ),
    ] {
        builder.declare_relation(
...
fn declare_change_fixture(builder: &mut RegistryBuilder) {
    ...
    builder.declare_enum(EnumDecl::platform(
        ...
        ["insert", "update", "delete", "rename"]
            .into_iter()
            .map(|name| EnumMember::new(name, "operation"))
            .collect(),
    ));
    builder.declare_relation(
        RelationDecl::new(
        ...
        .pk(&["change_set_id"])
        .columns(vec![
            ColumnSpec::key("change_set_id", LogicalType::id(), "identity"),
            ColumnSpec::payload("base_revision_id", LogicalType::id(), "base"),
            ColumnSpec::payload("author", LogicalType::Text, "author"),
            ColumnSpec::payload("message", LogicalType::Text, "message"),
            ColumnSpec::payload("created_at", LogicalType::Timestamp, "time"),
        ]),
    );
    let row = || {
    ...
    builder.declare_relation(
        ...
        .columns(vec![
            ColumnSpec::key("change_set_id", LogicalType::id(), "identity"),
            ColumnSpec::key("ordinal", LogicalType::U32, "order"),
            ColumnSpec::payload("op", LogicalType::enumeration("ChangeOpKind"), "kind"),
            ColumnSpec::payload("relation_id", LogicalType::id(), "relation"),
            ColumnSpec::payload("row_key", row(), "key"),
            ColumnSpec::payload("row", row(), "replacement").optional(),
            ColumnSpec::payload(
                "precondition",
                LogicalType::Text,
                "unsupported textual guard",
            )
            .optional(),
        ]),
    );
...
async fn assert_wrong_change_header_rejected(
    ...
    let error = catalog
        ...
        .await
        .unwrap_err();
    assert!(matches!(error, CatalogError::Admission { reason, .. } if
...
#[tokio::test]
async fn durable_stage_hints_require_exact_inputs_and_actual_successful_pass_records() {
    let reg = stage_registry();
    let store = Arc::new(InMemory::new());
    let catalog = catalog(
        Arc::clone(&store),
    ...
    let model = catalog
        ...
        .unwrap();
    let stage = catalog
        .publish_bundle(stage_draft(&reg, &model), &cancel)
        .await
        .unwrap();
    let inputs = BTreeMap::from([("input".to_owned(), Some(Arc::clone(&model)))]);
    let key = ContentHash::from_bytes([9; 32]);
    assert!(catalog.stage_hints(key, &cancel).await.unwrap().is_empty());
    let context = hint_context();
    for status in ["failed", "ok"] {
        let record = catalog
            .publish_sidecar(
                sidecar_draft(
                    ...
                    vec![
                        Cell::Id(pse_ids::SemanticId::from_bytes([9; 16])),
                        Cell::Id(reg.pass("P0").unwrap().id),
                        Cell::text("1"),
                        Cell::Hash(stage.snapshot_id().0),
                        Cell::text(status),
                    ],
                ),
                &cancel,
            ...
            .unwrap();
        let result = catalog
            .write_stage_hint_with_context(key, &inputs, &stage, &record, Some(&context), &cancel)
            .await;
        if status == "failed" {
        ...
        assert_retained_hint_ownership(&store, &reg, key, &hint).await;
        catalog
            .write_stage_hint_with_context(key, &inputs, &stage, &record, Some(&context), &cancel)
            .await
            .unwrap();
        assert_eq!(
            catalog.stage_hints(key, &cancel).await.unwrap(),
            vec![hint.clone()]
        );
        catalog
            .open_stage_hint(&hint, &inputs, &cancel)
            .await
            .unwrap();
        assert!(
            catalog
                .open_stage_hint(&hint, &BTreeMap::new(), &cancel)
                .await
                .is_err()
        );
        let mut forged = hint.as_ref().clone();
        forged
            .inputs
            .get_mut("input")
            .unwrap()
            .as_mut()
            .unwrap()
            .manifest_checksum = pse_ids::EncodingChecksum(ContentHash::NIL);
        assert!(
            catalog
                .open_stage_hint(&forged, &inputs, &cancel)
                .await
                .is_err()
        );
    }
}
fn hint_context() -> pse_catalog::store::stage_context::StageContext {
    use pse_catalog::store::stage_context::{StageContext, StagePolicy, StageSource};
    StageContext {
        declarations: vec![(
            "reference.fixture".to_owned(),
            vec![vec!["text:actual".repeat(64)]],
        )],
        sources: vec![StageSource {
            package_id: pse_ids::SemanticId::NIL,
            document_id: pse_ids::SemanticId::NIL,
            path: "package/model.yaml".to_owned(),
            text: "actual source bytes; ".repeat(1024),
        }],
        policies: BTreeMap::from([(
            "policy".to_owned(),
            StagePolicy {
                policy_id: pse_ids::SemanticId::NIL,
                relation: "reference.policy@1".to_owned(),
                rows: vec![vec!["text:actual policy".to_owned()]],
            },
        )]),
        engine: Some(pse_catalog::session::SessionSemantics {
            engine_version: "fixture engine".to_owned(),
            arrow_version: "fixture arrow".to_owned(),
            profile: pse_catalog::session::phase0_reference_profile(),
            settings: BTreeMap::from([
                ("set".to_owned(), Some("actual setting".to_owned())),
                ("absent".to_owned(), None),
            ]),
            functions: BTreeMap::from([("scalar".to_owned(), vec!["actual function".to_owned()])]),
        }),
    }
}

async fn assert_retained_hint_ownership(
    store: &Arc<InMemory>,
    reg: &Arc<Registry>,
    key: ContentHash,
    expected: &pse_catalog::store::stage::OwnedStageHint,
) {
    let budget = FixedBudget::new(16 << 20);
    let reader = catalog(Arc::clone(store), Arc::clone(reg), budget.clone());
    let hints = reader
        .stage_hints(key, &CancellationToken::default())
        .await
        .unwrap();
    assert_eq!(hints.len(), 1);
    assert_eq!(&hints[0], expected);
    let used = budget.reserved();
    assert!(used > expected.context.as_ref().unwrap().sources[0].text.len());
    let retained = hints[0].clone();
    assert_eq!(
        budget.reserved(),
        used,
        "immutable clone shares allocation and lease"
    );
    drop(hints);
    drop(reader);
    assert_eq!(
        budget.reserved(),
        used,
        "last detached hint retains all context data"
    );
    assert_eq!(retained.context, expected.context);
    drop(retained);
    assert_eq!(budget.reserved(), 0);
    let tiny = FixedBudget::new(1);
    let reader = catalog(Arc::clone(store), Arc::clone(reg), tiny.clone());
    assert!(
        reader
            .stage_hints(key, &CancellationToken::default())
            .await
    ...
    assert_eq!(
        tiny.reserved(),
        0,
        "failed bounded decode releases temporary ownership"
    );
}
...
async fn stage_admission_requires_actual_parents_exact_producer_and_every_output() {
    ...
    foreign
        .read_manifest(
            stage.manifest_ref(),
            &stage_draft(&reg, &reopened).context,
            &cancel,
        )
        .await
        .unwrap();
...
#[tokio::test]
async fn actual_document_inventory_and_source_extents_precede_snapshot_admission() {
    ...
    let cancel = CancellationToken::default();
    let source = b"model x";
    let document = pse_ids::SemanticId::from_bytes([1; 16]);
    assert!(
        catalog
            .publish_bundle(document_draft(&reg, source, 7), &cancel)
            .await
            .is_err()
    );
    let receipt = catalog
        .put_document(document, source, &cancel)
        .await
        .unwrap();
    assert_eq!(receipt.bytes, 7);
    assert_eq!(
        catalog
            .read_document(document, receipt.content_hash, &cancel)
            .await
            .unwrap()
            .as_ref(),
        source
    ...
    assert!(
        catalog
            .publish_bundle(document_draft(&reg, source, 8), &cancel)
            .await
            .is_err()
    );
    ...
    assert!(
        catalog
            .publish_bundle(document_draft(&reg, b"changed source", 7), &cancel)
            .await
            .is_err()
    );
    let path = pse_catalog::store::layout::document_path(
        document,
        &pse_ids::EncodingChecksum(receipt.content_hash),
    );
    store.put(&path, b"corrupt".to_vec().into()).await.unwrap();
    assert!(
        catalog
...
async fn sidecar_invariants_require_actual_evaluation_on_publish_and_restore() {
    ...
    let unchecked = catalog(store, reg.clone(), budget.clone());
    let spec = reg.relation("authored.items").unwrap();
        ...
        Box::pin(async move {
            ...
            let rows =
                pse_relations::cells::cells_from_batch(reg, documents, &candidates[&documents.key])
                    .unwrap();
            let [Cell::Id(id), Cell::Hash(hash)] = rows[0].as_slice() else {
                return Err(CatalogError::Membership {
                    reason: "fixture document row shape is invalid".to_owned(),
                });
            };
            let source = catalog.read_document(*id, *hash, cancel).await?;
            if source.as_ref() != b"model x" {
                return Err(CatalogError::Membership {
                    reason: "unrecognized fixture document".to_owned(),
                });
            }
            let expected = document_draft(reg, &source, 7);
...
async fn generic_invariant_success_cannot_certify_stage_producer_on_write_or_read() {
    ...
    let parent = guarded
        ...
        .await
        .unwrap();
    let context = stage_draft(&reg, &parent).context;
    for result in [
        guarded
            .publish_bundle(stage_draft(&reg, &parent), &cancel)
            .await,
        guarded
...
async fn semantic_refusal_cancellation_and_existing_corruption_cannot_move_head() {
    ...
    store
        .put(&path, bytes::Bytes::from_static(b"corrupt").into())
        .await
        .unwrap();
    assert!(
        catalog

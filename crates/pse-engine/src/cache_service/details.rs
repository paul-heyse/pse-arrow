...
impl NativeCacheService {
    ...
    pub fn inspect_entries(&self, limit: usize) -> Result<CacheEntries> {
        ...
        envelope_rows(&self.syntax, &mut rows, limit, &owner, budget)?;
        ...
        for component in self.components() {
            component.details(&mut rows, limit, &owner, budget)?;
        }
...
/// Reserve conservative staging before cloning a native cache inventory.
/// # Errors
/// Inventory size overflow or shared-pool staging refusal.
pub fn reserve_inventory(owner: &MemoryReservation, retained: usize, budget: usize) -> Result<()> {
    let bytes = retained
        .checked_mul(4)

...
use super::{EngineSession, engine_session::engine};
use crate::{
    EngineError,
...
impl EngineSession {
    pub(super) fn bind_targets(&self, plan: &LogicalPlan) -> Result<Self, EngineError> {
        ...
        let memory =
            pse_columnar::MemoryConsumer::new("session:command-targets").register(&self.pool);

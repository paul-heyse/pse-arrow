...
mod native;
mod tables;
use super::materialized::ImmutableTable;
...
impl PreparedProviderCapture {
    ...
    pub async fn execute(
    ...
    ) -> Result<CapturedProvider, EngineError> {
        ...
        let table = Arc::new(ImmutableTable {
            schema: self.schema,
            defaults: self.defaults,
            constraints: Some(self.constraints),
            data: completion.batches().to_vec(),
        });
...
pub struct CapturedProvider {
    table: Arc<ImmutableTable<Vec<pse_columnar::owned_buffer::OwnedRecordBatch>>>,
    registry: Arc<pse_schema::Registry>,
}
impl CapturedProvider {
    /// Completed captured buffers. Their owners are independent of producing plans.
    pub fn batches(&self) -> &[pse_columnar::owned_buffer::OwnedRecordBatch] {
        &self.table.data
    }
...
impl EngineSession {
    ...
    pub fn prepare_provider_capture(
        ...
        cancel: &CancellationToken,
    ) -> Result<PreparedProviderCapture, EngineError> {

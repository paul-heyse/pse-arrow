...
use super::{EngineSession, PreparedComputation};
...
impl PreparedCatalogResolution {
    ...
    pub async fn execute(self, cancel: &CancellationToken) -> Result<EngineSession, EngineError> {
...
#[derive(Debug)]
pub struct ResolutionGeneration {
    ...
    /// Qualified references covered by the completed metadata generation.
    pub references: BTreeSet<TableReference>,
    /// Consistency guaranteed by the actual metadata owner.
    pub consistency: ResolutionConsistency,
    /// Whether the completed generation covers all references in its namespace.
    pub exhaustive: bool,
}
...
struct Resolve {
    ...
    consistency: ResolutionConsistency,
    output: Mutex<Option<EngineSession>>,
...
impl EngineSession {
    ...
    ) -> Result<PreparedCatalogResolution, EngineError> {
...
impl Resolve {
    ...
    fn execute<'a>(
        ...
        session: &'a EngineSession,
        cancel: &'a CancellationToken,
    ) -> BoxFut<'a, Result<RecordBatch, EngineError>> {
        Box::pin(async move {
            let revision = cancel
            ...
            let reservation = pse_columnar::MemoryConsumer::new("provider:resolution-names")
...
async fn bind_resolved(
    ...
    cancel: &CancellationToken,
) -> Result<(EngineSession, u64), EngineError> {

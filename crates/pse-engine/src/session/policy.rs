...
impl EffectivePolicy {
    ...
    pub(super) fn admit(&self, effects: &BTreeSet<OperationEffect>) -> Result<(), EngineError> {
...
impl EngineSession {
    ...
    pub fn with_policy(&self, policy: ProviderPolicy) -> Result<Self, EngineError> {
    ...
    pub(super) fn compose_policy(&self) -> Result<EffectivePolicy, EngineError> {
    ...
    pub(super) fn admit_effects(
        ...
        plan: &LogicalPlan,
        varying: bool,
    ) -> Result<BTreeSet<OperationEffect>, EngineError> {
        ...
        let reservation =
            pse_columnar::MemoryConsumer::new("session:effect-discovery").register(&self.pool);
...
pub(super) fn policies_extent(policies: &[ProviderPolicy]) -> Result<usize, EngineError> {

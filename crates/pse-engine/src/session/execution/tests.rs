...
use super::*;
use crate::session::EngineFactory;
...
use pse_schema::model::provider::{OperationPurpose, ProviderPolicy};
use std::sync::Mutex;
...
fn session(capture: Arc<Capture>, cancel: &CancellationToken) -> EngineSession {
    ...
    .with_observation(crate::session::assurance::ObservationPolicy::Contract)
...
async fn native_algorithms_keep_functions_configuration_scopes_and_resource_ceiling() {
    ...
    let next = NativeExecutionContext::execution_state(&state, &cancel).unwrap();
    ...
    assert!(Arc::ptr_eq(services.pool(), next_services.pool()));
    ...
    assert!(
        pse_columnar::MemoryConsumer::new("too-large-native-workspace")
...
#[test]
fn invocation_resources_share_only_the_owned_attempt() {
    let pool: Arc<dyn MemoryPool> = Arc::new(pse_columnar::GreedyMemoryPool::new(4096));
    let first = InvocationResources::default();
    let second = InvocationResources::default();
    assert_ne!(first.scope, second.scope);
    assert_eq!(first.scope, first.scope.clone());
    let a = first.get(&pool, || Ok(731_u64)).unwrap();
    let b = first.get(&pool, || Ok(0_u64)).unwrap();
    let c = second.get(&pool, || Ok(219_u64)).unwrap();

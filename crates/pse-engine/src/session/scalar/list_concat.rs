...
impl PartialEq for TypedConcat {
    fn eq(&self, other: &Self) -> bool {
        Arc::ptr_eq(&self.pool, &other.pool) && Arc::ptr_eq(&self.native, &other.native)
    }
}
...
impl ScalarUDFImpl for TypedConcat {
    ...
    fn invoke_with_args(&self, mut args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        let reservation =
            pse_columnar::MemoryConsumer::new("query:typed-list-concat").register(&self.pool);
        ...
        let actual = pse_columnar::owned_buffer::retained_buffer_bytes(&batch).map_err(external)?;
        ...
        if scalar {
            pse_schema::literal::scalar_from_array(owned.column(0))

...
pub fn register(
    ...
    builder
        ...
        .extend([
            pse_relations::identity::function(),
            Arc::clone(&LITERAL),
            Arc::clone(&NAMED_ID),
            Arc::clone(&ID_LIST),
            nonnull::function(),
            nonnull::nullable_function(),
            list_field::function(Arc::clone(&pool)),
            list_concat::function(Arc::clone(&pool)),
            preserve_field::function(Arc::clone(&pool)),
            retain_metadata::function(),
            index_tuple::function(pool),
        ]);
    builder
}

/// Encodes an actual typed value without `serde_json::Value` reconstruction or float normalization.
...
impl ScalarUDFImpl for Codec {
    ...
    fn return_field_from_args(&self, args: ReturnFieldArgs<'_>) -> Result<FieldRef> {
        match self.kind {
            Kind::Literal => {
                ...
                pse_schema::literal::admit_type(field.data_type())
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        for row in 0..args.number_rows {
            ...
            text.push_str(
                &pse_schema::literal::to_json(array.as_ref(), &args.arg_fields[0], row)
                    .map_err(pse_columnar::external)?,
            );
            let length = output
                .values_slice()

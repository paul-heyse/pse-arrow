...
impl SnapshotCatalogList {
    ...
    pub(crate) fn freeze(&self) {
        ...
        for catalog in self
            ...
            .values()
        {
            if let Some(catalog) = catalog.downcast_ref::<super::catalog::SnapshotCatalog>() {
                catalog.freeze();
            }
        }
    }
...
impl CatalogProviderList for SnapshotCatalogList {
    fn register_catalog(
        ...
        if self.frozen.load(Ordering::Acquire) {
            // CatalogProviderList has no fallible registration method. A read
            // selection leaves the namespace intact; commands use a private fork.

...
fn collect(documents: &mut [Document]) -> Result<Vec<Pending>, AuthoringError> {
    ...
    for (index, document) in documents.iter_mut().enumerate() {
        ...
        for (key, _) in keys {
            ...
            {
                return Err(AuthoringError::UnknownKey {
                    at: document
                        .spans
    ...
    for column in &spec.columns {
        if row.section.source_column(column) == SourceColumn::PackageContext {
            ...
            if let Some(value) = row.value.get(column.name()) {
                ...
                if crate::ids::parse_id(supplied, row.at)? != package.package_id {
                    return Err(contract(
                        Some(row.at),
...
fn parent(row: &Pending) -> Result<Option<SemanticId>, AuthoringError> {
    ...
    match row.value.get(column) {
        ...
        Some(value) => value
            ...
            .and_then(|text| crate::ids::parse_id(text, row.at))
            .map(Some),
    }
}
...
fn identity(
...
) -> Result<Option<authored::entities::Row>, AuthoringError> {
    ...
    let Some(kind) = row.section.entity_kind else {
        let text = explicit.ok_or_else(|| AuthoringError::MissingId {
            at: row.at,
            kind: row.section.relation.to_owned(),
            name: "unnamed semantic record".to_owned(),
        })?;
        let id = crate::ids::parse_id(text, row.at)?;

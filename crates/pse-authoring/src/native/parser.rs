...
use crate::{AuthoringError, ParseBudget, document::load_package_sources_owned};
use datafusion::{
    arrow::{
...
impl Parser {
    ...
    fn parse(&self, package: &[u8], sources: &dyn Array) -> Result<RecordBatch> {
        ...
        let bundle = load_package_sources_owned(
            ...
            self.binding.budget,
            self.binding.reserver.as_ref(),
            &self.binding.cancel,
        )
        .map_err(external)?;
        if bundle.bundle().package.package_id.as_bytes().as_slice() != package {
            return Err(external(AuthoringError::Contract {
                at: None,
                reason: "source package grouping disagrees with its actual header identity".into(),
            }));
        }
...
impl ScalarUDFImpl for Parser {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        let mut reservation = binding.reserver.open("authoring:native-parser-output");
        reservation
            .try_grow(
                args.number_rows
                    .checked_add(1)

...
//!
//! Phase-0 placeholder: `pse-ids` has no canonicalizer yet, so this measures `std`'s hasher over
//! a small slice. It exists to keep the bench target compiling and running in CI from the
//! first commit; the real group hashes canonical IPC bytes under the `pse.canon.v2`
...
const PAYLOAD: &[u8; 32] = b"pse.canon.v2 placeholder payload";

...
**Codebase context:** typed relations and their registry already supply authority in `pse-arrow`; the native-plan transition is governed by [blueprint D1, D10, §3.3.3, §4–§5](../authoritative_design/blueprint.md) and [Plan 05](../plans/05-native-logical-plan-hard-pivot.md). These DataFusion mechanisms can expose and execute that authority. Neither a catalog nor session settings automatically becomes a replacement source of domain truth.

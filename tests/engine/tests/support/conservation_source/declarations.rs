...
pub(super) fn template(balance: Balance) -> Value {
    ...
    let bindings=[("p","phase"),("j","species"),("e","element")].map(|(name,source)|json!({"template_id":id(60),"name":name,"source":source,"domain_id":null,"parameter_name":null}));
    ...
    let contributions=[(50,"inflow","into_scope"),(51,"outflow","out_of_scope")].map(|(contribution,name,orientation)|json!({"template_id":id(60),"contribution_decl_id":id(contribution),"name":name,"law_family":balance.family(),"expression":format!("{name}{suffix}"),"orientation":orientation,"scope":"self","guard_id":null,"doc":"Complete indexed source contribution."}));
    ...
    let contracts=[50,51].map(|contribution|json!({"contribution_decl_id":id(contribution),"indexed_by":axes,"quantity_type_id":balance.source_type(),"subject_kind":balance.subject(),"subject_axis":if material{Some(1)}else{None},"phase_axis":if axes.is_empty(){None}else{Some(0)},"subject_id":null,"phase_id":null}));
...
pub(super) fn materials(options: &Options) -> Value {
    let species=[(100,"hydrogen","H2",0.002016),(101,"oxygen","O2",0.031998)].map(|(species,name,formula,mw)|json!({"id":id(species),"name":name,"formula":formula,"mw":options.molecular_weight.then_some(mw),"component_type":"Component","charge":0,"dissociation_species":null,"valid_phase_types":["vaporPhase","liquidPhase"],"doc":"Known diatomic stoichiometry with optional explicit kg/mol source value."}));

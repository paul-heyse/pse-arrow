...
pub struct DeltaTableConfig {
    ...
    /// Maximum commits to advance a stale kernel CRC. Zero disables incremental
    /// CRC replay; active-file enumeration still uses native action replay.
    #[serde(default)]
    #[delta(skip)]
...
impl PartialEq for DeltaTableConfig {
    fn eq(&self, other: &Self) -> bool {
        self.require_files == other.require_files
            ...
            && self.crc_replay_max_commits == other.crc_replay_max_commits
    }
}
...
impl DeltaTableBuilder {
    ...
    /// Bound native incremental CRC replay without changing file replay semantics.
    pub fn with_crc_replay_max_commits(mut self, max_commits: u64) -> Self {
        self.table_config.crc_replay_max_commits = max_commits;
        self
    }

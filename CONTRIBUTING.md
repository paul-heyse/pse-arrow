...
authoritative sections and decision process; current work, when there is any, lives in
an active plan.
...
- `docs/authoritative_design/README.md` — architecture entrypoint. Focused numbered pages
  under `sections/` describe the current system; section numbers are stable citation
  identities (`blueprint §14.3`). Design amendments need an ADR and a revision row in
  `blueprint.md`, which also maps former single-file anchors. Retained ADRs explain why;
  completed plans, resolved reviews and obsolete decisions retire to Git history (ADR-0096).
...
(`just adr-supersede`). A record whose rationale no longer explains the current system is
retired to Git history rather than rewritten (ADR-0096). Deferred items live as rows in
`docs/adr/register.md` with an observable trigger and a check; `just register-check`
...
establish implementation. When a plan closes, move enduring meaning to its contract owner;
the completed plan and its resolved reviews then retire (Git history keeps them).
...
- New files are lowercase kebab-case. Exceptions: the root governance files and the
  design-review skill's output names.
- YAML front matter is required on ADRs, plans, and capability maps.
- Cite as `blueprint §14.3`, `ADR-0082`, `AP-06`, `DP-09`, `PS-10`, `G4`. Section numbers are stable: insert

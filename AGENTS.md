...
| `just codegen-check` | the committed generated sources match what the generator emits now | nothing about whether the generator is correct |
...
| `just parity` | behaviour matches `idaes-pse==2.12.0` on the cases exercised | nothing about cases not exercised, or other IDAES versions |
...
- **Claude Code** reads `CLAUDE.md`, which imports this file. Settings, hooks and
  path-scoped rules are in `.claude/`.
- **Codex** reads this file directly. It runs here with broad filesystem access, so this
  file is its only guidance — be correspondingly careful with destructive commands.
- **Skills are canonical in `.codex/skills/`**; `.claude/skills` is a symlink to it.
  **Agent definitions are canonical in `.claude/agents/`**; `.codex/agents` is a symlink
  to it. One copy each, so the two runtimes cannot drift.
- `just lint-agents` checks that both symlinks resolve and that every repository path
  named in these files still exists.
